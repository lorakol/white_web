...
function common($smarty, $start_session = true)
{
    ...
    global $config_vat_rates;
    $smarty->assign("config_vat_rates", $config_vat_rates);
...
//	$meta_format = common::getCachedObject("base_meta_info", array("page" => $self_noext));
    ...
    $auth = new auth();
    global $logged_in, $is_mod, $is_aff;
...
//echo $logged_in."-------";
    ...
    if ($logged_in) {
        ...
        if (users::isBlocked($crt_usr) && $self_noext != "access_restricted") {
            header("Location: " . $config_live_site . "/access_restricted.php");
            exit(0);
        }
    ...
    do_action("after_auth", array($smarty, $self_noext));

    common::getInterfaceCachedObjects();

    if (!$logged_in || $is_admin) $crt_group = '1000';
    else {
        $crt_group = $usr_sett['group'];
    }
    ...
    if ($logged_in && $config_vars['credits_enabled']) {
        ...
        if ($allowed) {
            ...
            if ($no_credits) $credits_amount = add_currency(format_price(users::getNoCreditsNoFormat($crt_usr) * $credits_settings['unit']));
