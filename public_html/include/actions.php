...
$arr_action= array("add", "delete", "sold", "unsold", "rented", "unrented", "favourite", "report", "delete_file", "enable", "disable", "mark_read", "mark_unread", "email-on", "email-off");
...
switch ($object) {
	case 'listing':
		...
		else if(!strcmp($action,"email-on")) $listing->setEmailOn($id);
		else if(!strcmp($action,"email-off")) $listing->setEmailOff($id);
