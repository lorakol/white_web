...
class cache {
	...
	public function __construct()
    {
		...
		$this->mobile_cache_path = $config_abs_path."/temp_mobile";
    }
	...
	function readCache($cache_name, $lang_name='', $param=null) {
		...
		$extra_param = '';
		if($param) {
			foreach($param as $key => $value) $extra_param.=$value."_";
			$extra_param = trim($extra_param, "_");
		}
	...
	function clearAllCacheFiles(){
		...
		// empty mobile cache
		$folders_h = dir($this->mobile_cache_path);
		while ($f = $folders_h->read()) {
			// only cache files
			$ext = getExtension($f);
			if( $ext != "php") continue;
