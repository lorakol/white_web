...
class seo {
	...
	function makeSearchCategoryLink ($id, $name='', $slug='') {
		...
		if(!$name || $crt_lang!=$default_lang) $name = cleanHtml(categories::getDefaultName($id));
		...
		if(isset($search_array) && $search_array) {
			...
			if(isset($search_array['category']) && $search_array['category'] && $current != "cat") { 
				if(!$seo_settings['sef_legacy_mode']) {
					...
					$default_lang = languages::getDefault();
					require_once $config_abs_path."/classes/categories.php";
					if(!$category_name || $crt_lang!=$default_lang) $category_name = cleanHtml(categories::getDefaultName($search_array['category']));
			...
			foreach($search_array as $key => $value) {
				...
				if(($key=="order" && $value=="date_updated") || ($key=="order_way" && $value=="desc")) continue;
	...
	function makeDetailsLink($id, $title='', $search_id=0, $category_id=0, $slug='') {
		...
		// legacy mode
		if(isset($title) && $title) $url_title = _urlencode_title($title);
		else { 
			global $config_abs_path;
	...
	function makeSubdomainSearchCategoryLink($id, $name='', $subdomain='') {
		...
		global $crt_lang;
		$default_lang = languages::getDefault();
		if(!$name || $crt_lang!=$default_lang) {
			require_once $config_abs_path."/classes/categories.php"; 
			$name = cleanHtml(categories::getDefaultName($id));
		}
		$search_link = $config_live_site."/".$id."-"._urlencode($name)."/".$this->links["listings"];
...
if($settings['enable_locations'] && $settings['enable_subdomains']) {
	...
	function getRecentAdsMetaInfo() {
		...
		// non canonical links
		if( (!empty($_GET['order']) && $_GET['order'] != "date_updated") || ( !empty($_GET['order_way']) && $_GET['order_way'] != "desc") ) {
			global $config_live_site;
	...
	function getUserPageMetaInfo($id) {
		...
		// canonical links
		if( (!empty($_GET['order']) && $_GET['order'] != "date_updated") || ( !empty($_GET['order_way']) && $_GET['order_way'] != "desc") || $canonical_subdomains ) {
	...
	function getSearchMetaInfo($smarty) {
		...
		// non canonical links
		if( (!empty($post_array['order']) && $post_array['order'] != "date_updated") || ( !empty($post_array['order_way']) && $post_array['order_way'] != "desc") ) {
