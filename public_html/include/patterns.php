...
*/
	...
	$pattern_youtube_embed = '/^<object( width="\d+")?( height="\d+")?><param name="movie" value="http(s):\/\/(www\.)?youtube(-nocookie)?.com\/(.*)<\/object>$/i';

	global $pattern_youtube_iframe_embed;
	$pattern_youtube_iframe_embed = '/^<iframe(.*)( src="http(s):\/\/(www\.)?youtube(-nocookie)?.com\/(.*)")(.*)><\/iframe>$/i';
	...
	$youtube_default_embed_code = '<iframe width="%width" height="%height" src="https://www.youtube.com/embed/%video" frameborder="0" allowfullscreen></iframe>';
	...
	function formatVideo($str) {
		...
		if(preg_match($pattern_youtube_url, $str)) {
			...
			parse_str( parse_url( $str, PHP_URL_QUERY ), $output );

			$arr = array("%width" => $w, "%height" =>$h, "%video" => $output['v'], "http://" => "https://");
