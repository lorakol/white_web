...
function my_session_start() {
	...
	// server should keep session data for AT LEAST this time
	$timeout = $settings['accounts_session_timeout']*60;
	...
	if(($settings['enable_locations'] && $settings['enable_subdomains'])) {
		...
		global $main_domain;
		session_set_cookie_params($timeout, '/', '.'.$main_domain);
	}
	else

	// each client should remember their session id for EXACTLY this time
...
function _get_magic_quotes_gpc() {
	...
	if (version_compare(phpversion(), "5.4.0", ">=")) return 0;
...
function escape($str, $no_trim=0) {
	...
	global $force_magic_quotes;
	$magic_quotes = 0;
	if(_get_magic_quotes_gpc()) $magic_quotes=1;
	...
	if(!$no_trim)
   	...
   	// remove the 4 byte characters
	// if not removed, anything after a 4 byte character will not be stored in MySQL
	global $appearance_settings;
	$charset = $appearance_settings['charset'];
	if($charset=="UTF-8")
		$clean_str = preg_replace('/[\xF0-\xF7].../s', '', $clean_str);
...
function _urlencode($str, $replace_signs = 1, $no_chars = 0, $all_maps=0) {
	...
	if($replace_signs) {
		...
		// unsafe characters: " ", "?", "<>", "[]", "{}", "|", "\", "^", "~", "%", "#", "`"
		// other signs: "!", ".", "(", ")", ", "'"
		//$str = preg_replace('/[^\x20-\x7E]/', '', $str);  // letters, digits, punctuation marks, and a few miscellaneous symbols
...
function _urlencode_title($str) {

	$str = strip_tags($str);
	$str = characters_map($str, 1);
	$str = strtolower($str);   // Convert to lowercase
...
function format_price_field($str, $decimals='', $th_separator='', $point='') {
	...
	// remove .00 at the end if no decimals
	if($decimals) $result = cleanPrice($result, $decimals, $point);
...
function cleanPrice($str, $decimals, $point) {

	$zero_dec_str = $point;
	for($a=0; $a<$decimals;$a++) $zero_dec_str.="0";
	return(str_replace($zero_dec_str, '', $str));
...
$default_fields_types = array("textbox", "textarea", "htmlarea", "menu", "multiselect", "radio", "radio_group", "checkbox", "checkbox_group", "depending", "url", "email", "date", "file", "image", "logo", "youtube", "google_maps", "terms", "price", "username", "password", "user_email", "phone", "whatsapp", "twitter", "section", "video", "audio","button");
...
function createActionsArray() {
    ...
    return array( "ad_id" => 0, "pkg_id" => 0, "credits_pkg_id" => 0, "newad" => array( 'value' => 0 , 'price' => 0) , "renewad" => array( 'value' => 0 , 'price' => 0), "featured2" => array( 'value' => 0 , 'price' => 0), "featured" => array( 'value' => 0 , 'price' => 0), "highlited" => array( 'value' => 0 , 'price' => 0), "priority" => array( 'value' => 0 , 'price' => 0), "urgent" => array( 'value' => 0 , 'price' => 0), "video" => array( 'value' => 0 , 'price' => 0), "url" => array( 'value' => 0 , 'price' => 0), "newpkg" => array( 'value' => 0 , 'price' => 0), "renewpkg" => array( 'value' => 0 , 'price' => 0), "new_creditspkg" => array( 'value' => 0 , 'price' => 0), "store" => array( 'value' => 0 , 'price' => 0), "discount_code" =>'' );
