...
function releasePayments() {
	...
	$array_affiliates = $db->fetchAssocList("select ".TABLE_AFFILIATES.".* from ".TABLE_AFFILIATES." left join ".TABLE_AFFILIATES_REVENUE." on ".TABLE_AFFILIATES.".affiliate_id=".TABLE_AFFILIATES_REVENUE.".affiliate_id where ".TABLE_AFFILIATES_REVENUE.".released=0");
	
	// for each affiliate
	foreach($array_affiliates as $aff) {

		$total = $db->fetchRow("select sum(`amount`) from ".TABLE_AFFILIATES_REVENUE." where `affiliate_id`='{$aff['affiliate_id']}' and released=0");
	
		$payment_id = $this->addToPaymentHistory($aff['affiliate_id'], $aff['affiliate_paypal_email'], $total);
