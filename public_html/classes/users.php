...
class users {
	...
	function Enable($id) {
		...
		$array = $db->fetchAssoc("select ".TABLE_USERS.".* from ".TABLE_USERS." where ".TABLE_USERS.".`id` =$id");

		$res=$db->query('update '.TABLE_USERS.' set `active`=1, `pending`=0 where id="'.$id.'"');
		...
		} // END SEND EMAIL IF THE USER REQUIRES ADMIN VERIFICATION
	...
	function Disable($id) {
		...
		$res=$db->query('update '.TABLE_USERS.' set `active`=0, `pending`=0 `activation`="" where id="'.$id.'"');
	...
	function activate_link($aname, $activation) {
		...
		$res=$db->query("select `id`, `active`, `pending`, `group`, `activation`".$str_extra." from ".TABLE_USERS." where $field like '$aname'");
		...
		// if activation is empty and active = 0 -> pending account
		require_once "classes/groups.php";
		if(!$result['activation'] && groups::getAdminVerification($result['group'])) { $this->addError('', $lng['users']['errors']['account_already_activated']); return 0; }
		...
		if (groups::getAdminVerification($result['group'])) {
			...
			$res=$db->query("update ".TABLE_USERS." set `activation`='', `pending`=1 where $field like '$aname'");
			return -1;
...
//???!!!!!!!!cleanStr???
	...
	function getUser($id, $not_formatted=0) {
		...
		$result=$db->fetchAssoc("select *, ".TABLE_USERS.".`ip` as register_ip, ".TABLE_USERS.".`id` as id, date_format(`registration_date`,'$date_format') as date_nice, ( ".TABLE_BLOCKED_IPS.".ip is not null ) as blocked, ( ".TABLE_FLAGGED_IPS.".ip is not null ) as flagged from ".TABLE_USERS." left join ".TABLE_BLOCKED_IPS." on ".TABLE_USERS.".ip = ".TABLE_BLOCKED_IPS.".ip  left join ".TABLE_FLAGGED_IPS." on ".TABLE_USERS.".ip = ".TABLE_FLAGGED_IPS.".ip where ".TABLE_USERS.".id='$id'");
	...
	function getLoginHistory($page,$no_per_page, $order, $order_way, $search='') {
		...
		$sql = "select $field, date_format(`last_login`,'$date_format') as last_login_nice from ".TABLE_USERS." $where order by $order $order_way limit $start, $no_per_page";
		...
		if($page==1 && !$search) {
			...
			$array_users[$i]['last_login_nice']=$login_info['date_login_nice'];
			$array_users[$i]['ip']=$login_info['ip'];
			$i++;
		}

		foreach ($array as $result) {
			$array_users[$i]=$result;
			$login_info=$auth->getLastLogin($result[$field]);
			if($login_info)
				$array_users[$i]['ip']=$login_info['ip'];
			else $array_users[$i]['ip']='';	
			$i++;
		}
		return $array_users;
	...
	function searchUsers($post_array, $page,$no_per_page,$order,$order_way) {
		...
		foreach($post_array as $key=>$value) {
			...
			switch($key) {
				...
				case "inactive":
					if($found) $where.=" and "; else $where = " where ";
					$where.=TABLE_USERS.".`active` = '0' ";
					$found = 1;
				break;
				case "verification":
					if($found) $where.=" and "; else $where = " where ";
					$where.=TABLE_USERS.".`admin_verification` = 1 ";
					$found = 1;
				break;
				case "pending":
					if($found) $where.=" and "; else $where = " where ";
					$where.=TABLE_USERS.".`pending`=1 ";
					$found = 1;
				break;
				case "inactive_pending":
					if($found) $where.=" and "; else $where = " where ";
					$where.=TABLE_USERS.".`pending`=1 and ".TABLE_USERS.".`active`=0";
					$found = 1;
				break;
				case "inactive_not_pending":
					if($found) $where.=" and "; else $where = " where ";
					$where.=TABLE_USERS.".`pending`=0 and ".TABLE_USERS.".`active`=0";
					$found = 1;
				break;
				case "date_from":
					if($found) $where.=" and "; else $where = " where ";
					$where .= TABLE_USERS.".`registration_date` >= '$value' ";
					$found = 1;
				break;
				...
				case "only_moderators":
					if($found) $where.=" and "; else $where = " where ";
					$where .= TABLE_USERS.".`moderator` = 1 ";
					$found = 1;
				break;

				case "only_stores":
					if($found) $where.=" and "; else $where = " where ";
					$where .= TABLE_USERS.".`store` = 1 ";
					$found = 1;
				break;

				case "only_affiliates":
					if($found) $where.=" and "; else $where = " where ";
					$where .= TABLE_USERS.".`affiliate` = 1 ";
					$found = 1;
				break;
				default:
		...
		if(!strstr($order, "`")) $order="`".$order."`";
		
		$sql="select ".TABLE_USERS.".*, $no_ads date_format(registration_date,'".$date_format."') as date, ".TABLE_ACTIONS.".`invoice` from ".TABLE_USERS."
$join_ads
...
//echo $sql;
		...
		require_once $config_abs_path."/classes/flagged_ips.php";
		...
		foreach ($array as $result) {
			...
			$array_users[$i]['ip_flagged'] = $array_users[$i]['flagged'] =  flagged_ips::isFlagged($array_users[$i]['ip']);
			$array_users[$i]['email_blocked'] = blocked_emails::isBlocked($array_users[$i]['email']);

			// ---------- blocked phones --------------
			$where_str='';
			if($result['group']) $where_str .= " and ( (`groups` REGEXP CONCAT('(^|,)(', REPLACE('{$result['group']}', ',', '|'), ')(,|$)') and `groups`!=-1)  or `groups` = 0 )";
...
//echo $sql;
	...
	function searchAffiliates($post_array, $page,$no_per_page,$order,$order_way) {
		...
		$sql="select ".TABLE_USERS.".*, date_format(registration_date,'".$date_format."') as date, ".TABLE_BLOCKED_IPS.".`ip` as blocked, ".TABLE_FLAGGED_IPS.".`ip` as flagged, ".TABLE_ACTIONS.".`invoice` from ".TABLE_USERS."
left join ".TABLE_BLOCKED_IPS." on ".TABLE_USERS.".`ip`=".TABLE_BLOCKED_IPS.".ip
left join ".TABLE_FLAGGED_IPS." on ".TABLE_USERS.".`ip`=".TABLE_FLAGGED_IPS.".ip
left join ".TABLE_USER_GROUPS." on ".TABLE_USERS.".`group`=".TABLE_USER_GROUPS.".`id`
left join ".TABLE_ACTIONS." on ".TABLE_USERS.".id=".TABLE_ACTIONS.".`user_id` and ( ".TABLE_ACTIONS.".`type` = 'store' )
$where
...
//echo $sql;
		...
		foreach ($array as $result) {
			...
			if($array_users[$i]['flagged']) $array_users[$i]['flagged']=1;
			else $array_users[$i]['flagged']=0;
	...
	function check_edit_info ($id, $mod) {
		...
		if( ($is_admin || ( $store && $ads_settings['dealers_can_edit_slug'])) && $seo_settings['enable_mod_rewrite'] && !$seo_settings['sef_legacy_mode'] ) {

				global $config_abs_path;
				require_once $config_abs_path."/classes/slugs.php";
				$user_slug = escape($_POST['user_slug']);
				$s = new slugs();
				if($s->slugExists($user_slug, $id)) $this->addError("user_slug", $lng['users']['errors']['slug_already_used']);
...
//_print_r($_POST);
	...
	function add($group='') {
		...
		if($is_admin || $is_mod) { 
			$clean['active']=1;
			$clean['pending']=0;
			if($admin_verification) $clean['pending']=1;
		}	
		else {
		...
		if($is_admin || $is_mod) array_push($insert_array, "pending");
...
//echo "last_id=".$last_id;
		...
		// add secret key for 2fa
		$this->regenerate2FAKey($last_id);
		...
		if($activate_via_email || $activate_via_sms || $clean['active']) { //  send mail to the user only if activation is needed or the account is active
			...
			if($activate_via_sms==1) {
				...
				$sg = new sms_gateways();
				$sg_settings = $sg->getGeneralSettings();
				
				$activate_account = 2;
				global $config_live_site;
				// add activation code to db record
				if($sg_settings['type']==1)
					// alphanumeric
				...
				if($phone_no) {
					...
					if($default) {
						...
						$gcl = new $class_name;
						$sent = $gcl->send($phone_no, $sg_settings['sms_content'].$activation_code, $last_id);
						// !!!!!!!!!!!!!
	...
	function edit_info($id) {
		...
		if( ($is_admin || ( $store && $ads_settings['dealers_can_edit_slug'])) && $seo_settings['enable_mod_rewrite'] && !$seo_settings['sef_legacy_mode'] ) {
				...
				$user_slug = escape($_POST['user_slug']);
				$s = new slugs();
				$s->replaceSlug($id, 'user', $user_slug);
	...
	function change_password($id, $remove_2fa=0) {
		...
		// disable 2FA
		if($remove_2fa)
			$res=$db->query('update '.TABLE_USERS.' set `enable_2fa` = 0 where id='.$id.';');
	...
	function edit_2fa($id) {
	
		global $db, $lng;
		$tfa = checkbox_value('enable_2fa');
		$res=$db->query('update '.TABLE_USERS.' set `enable_2fa` = '.$tfa.' where id='.$id.';');
		$this->setInfo($lng['useraccount']['settings_saved']);
		return 1;
	...
	function validAccount($aname,$passhash) {
		...
		if($user_array['active']==0) {
			...
			if(in_array("suspend_user", $modules_array) && $user_array['suspended']==1) {
				...
				global $config_abs_path;
				require_once $config_abs_path."/classes/groups.php";
				if(!$user_array['active'] && !$user_array['activation'] && groups::getAdminVerification($user_array['group'])) {
					// awaiting for admin verification
					$this->setLoginError ($lng['users']['info']['awaiting_admin_verification']);
					}
				else
		...
		if($security_settings['enable_2fa'] && $user_array['enable_2fa'] && $user_array['2fa_secret']) return 2;
		return 1;
...
//		$res=$db->query("select * from ".TABLE_USERS." where `username` like '$user' and `identity` like '$identity' and `active`=1");
	...
	function getTableFields() {
		...
		$extra_fields = array("registration_date_formatted", "listings", "blocked", "group_name", "invoice");
	...
	function exportUsers($type){
		...
		$sql="select ".TABLE_USERS.".*, date_format(registration_date,'".$date_format."') as `registration_date_formatted`, count(".TABLE_ADS.".user_id) as listings, ".TABLE_BLOCKED_IPS.".`ip` as blocked, ".TABLE_USER_GROUPS."_lang.`name` as `group_name`, ".TABLE_ACTIONS.".`invoice` from ".TABLE_USERS."
	...
	static function getNoCreditsNoFormat($id) {
	...
	static function hasAdminVerification($id) {

		global $db;
		$av = $db->fetchRow("select `admin_verification` from ".TABLE_USERS." where `id`='$id'");
		return $av;

	}

	function getNoAdminVerificationUsers() {

		global $db;
		$av = $db->fetchRow("select count(*) from ".TABLE_USERS." where `admin_verification`=1");
		return $av;

	}

	static function getUserIdByAuthName($auth_name) {

		global $db;
		$id=$db->fetchRow('select `id` from '.TABLE_USERS.' where username like "'.$auth_name.'" or email like "'.$auth_name.'"');
		if(!$id) return 0;
		return $id;

	}

	function regenerate2FAKey($id) {
		...
		global $db;
		$db->query("update ".TABLE_USERS." set `2fa_secret` = '$secret' where id='$id'");
		return $secret;
