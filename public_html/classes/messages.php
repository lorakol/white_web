...
class messages {
	...
	function markAsRead($id) {
		...
		$res=$db->query('update '.TABLE_MESSAGES.' set `read`=1 where id='.$id);
	...
	function markAsUnread($id) {
		...
		$res=$db->query('update '.TABLE_MESSAGES.' set `read`=0 where id='.$id);
	...
	function getMessages($post_array, $page,$no_per_page,$order,$order_way, $hide_pending=0) {
		...
		foreach($messages_array as $msg) {
			...
			$messages_array[$i]['flagged'] = $db->fetchRow("select count(*) from ".TABLE_FLAGGED_IPS." where ip like '{$messages_array[$i]['ip']}'");
			$i++;
		}
...
//_print_r($messages_array);
	...
	function add($to, $to_email, $from, $from_email, $from_phone, $ad_id, $message, $reply_to='', $pending = 0) {
		...
		global $db;
		$reply_str = ""; $starting_str="";$from_string="";
		if($reply_to) $reply_str = ", `reply_to` ='$reply_to'";
		if($this->starting) $starting_str = ", `starting` ='{$this->starting}'";
		if($from) $from_string="`from` = '$from',";
		$db->query("insert into ".TABLE_MESSAGES." set $from_string `from_email`='$from_email', `from_phone`='$from_phone', `to` = '$to', `to_email` = '$to_email', `date` = '$timestamp', `message` = '".escape($message)."', `ad_id` = '$ad_id', `ip`='$ip', `pending`='$pending'".$reply_str.$starting_str);
