...
class validator {
	...
	static function valid_price($str, $point='', $separator='')
	{

		global $appearance_settings;
		if(!$point) $point = $appearance_settings['price_format_point'];
		if(!$separator) $separator = $appearance_settings['price_format_separator'];
// 		$ereg_str = "/^([0-9]+)?(".$separator.")?([0-9]+)?(".$point.")?([0-9]+)?$/";
		$ereg_str = "/^(([0-9]+)?(".$separator.")?)+?([0-9]+)?(".$point.")?([0-9]+)?$/";
	...
	static function removeArgs($url) {
