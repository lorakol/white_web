...
class pictures {
	...
	function getPictures($ad_id) {
		...
		if(!empty($array)) {
			...
			foreach ($array as $row) {
				...
				$ss[$i]['width'] = $ss[$i]['height'] = 0;
				if($size) {
					$ss[$i]['width']=$size[0];
					$ss[$i]['height']=$size[1];
				}	
				$size_thmb=@getimagesize($thmb_target_folder.$row['picture']);
				$ss[$i]['thmb_width'] = $ss[$i]['thmb_height'] = 0;
				if($size_thmb) {
	...
	function getPicturesFormatted($ad_id, $pending_edited=0) {
		...
		foreach ($array as $row) {
			...
			if(!$size) $size = array('', '');
			$pst = array ("img" => array("src" => $url, "name" => $row['picture'],  "rename" => $row['picture'], "width" => $size[0], "height" => $size[1],  "title" => $row['title']));
	...
	function addToDB($ad_id, $file, $folder='', $pending_edited = 0) {
		...
		$last_order = $this->getMaxOrder($ad_id, $pending_edited);
 		...
 		if($ad_id && !$pending_edited) {
			...
			array_push($_SESSION['pictures'], array( "picture" => $file, "folder" => $folder, "title" => "" ));
			if($pending_edited)
				$this->saveAsPendingEdited($ad_id);
		}
	...
	static function addSessionsToDB($ad_id) {
		...
		$order_no = 1;
		foreach ($_SESSION['pictures'] as $p) {
			$res=$db->query("insert into ".TABLE_ADS_PICTURES." (`ad_id`, `picture`, `order_no`, `folder`, `title`) values ('$ad_id', '{$p['picture']}', '$order_no', '{$p['folder']}', '{$p['title']}')");
			$order_no++;
		}
	...
	function prepareRegenerate($only_missing=0) {
		...
		if($only_missing) return $picture;
	...
	function regeneratePicture($id, $watermark, $only_missing) {
		...
		if(!$only_missing || !file_exists($config_abs_path."/images/listings/".$folder_str."thmb/".$pic['picture']))
			$img->resize($pic['picture'], 'thmb', $ads_settings['thmb_width'], $ads_settings['thmb_height'], 0);
		if(!$only_missing || !file_exists($config_abs_path."/images/listings/".$folder_str."medThmb/".$pic['picture']))
			$img->resize($pic['picture'], 'medThmb', $ads_settings['med_thmb_width'], $ads_settings['med_thmb_height'], 0);
		if(!$only_missing || !file_exists($config_abs_path."/images/listings/".$folder_str."bigThmb/".$pic['picture']))	
		...
		if($mobile_settings['enable_mobile_templates']) {
			if(!$only_missing || !file_exists($config_abs_path."/images/listings/".$folder_str."mobile_thmb/".$pic['picture']))
				$img->resize($pic['picture'], 'mobile_thmb', $mobile_settings['mobile_thmb_width'], $mobile_settings['mobile_thmb_height'], 0);
			if(!$only_missing || !file_exists($config_abs_path."/images/listings/".$folder_str."mobile_bigThmb/".$pic['picture']))	
	...
	function getPictureByName($name, $ad_id) {
	
		global $db;
		$array = $db->fetchAssoc("select * from ".TABLE_ADS_PICTURES." where `picture` like '$name' and `ad_id` = ".$ad_id);
		return $array;
	...
	function setTitle($ad_id, $name, $title) {

		global $db;
		
		$db->query("update ".TABLE_ADS_PICTURES." set `title`='".escape($title)."' where `picture` like '$name' and `ad_id` = ".$ad_id);
		return $array;
	
	}

	function setPETitle($ad_id, $name, $title) {
