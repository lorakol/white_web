...
/*
	...
	* version 9
// sewt	* (c) 2017 OxyClassifieds.com (office@oxyclassifieds.com).
	*
*/
...
function getExportPath() {
	...
	if(!isset($this->export_path) || !$this->export_path) 
		return $config_abs_path.'/temp/';
	return $config_abs_path."/".$this->export_path;
...
function Export() {
	...
	if (!($this->f = fopen($this->getExportPath().$fname, 'w'))) {
...
//echo $sql;exit(0);
	...
	while ($row = $db->fetchAssocRes($arr)) {
		...
		if($this->data == "ad") {
			...
			$row['invoice'] = $db->fetchRow("select `invoice` from ".TABLE_ACTIONS." where `object_id`= '{$row['id']}' and (`type` like 'newad' or `type` like 'renewad') order by `date` desc limit 1");
...
function XMLExport() {
	...
	if($this->data == "ad") {
		$listings = new listings();
		$sql = $this->exportListings("xml");echo $sql."\n";
		$top_tag = "<listings>";
		$bottom_tag = "</listings>";
		$start_tag = "<listing>";
		$end_tag = "</listing>";
...
function FormXMLExport() {
	...
	function xml_parse_file($file_name, $pos=0, $encoding="UTF-8")
	{
		...
		if(!$this->scheduled) {
			//if(isset($this->crt_pos) && $this->crt_pos) $new_pos = $this->crt_pos;
			//else
...
//echo "end tag!!!";
			...
			$this->continue = 0;
//echo "pos=".xml_get_current_byte_index();
//echo "pos=".ftell($fp);
		}
	...
	function XMLImport($file_name) {
		...
		$import_id = $db->fetchRow("select LAST_INSERT_ID()");
		return $import_id;
	}
	...
	function CSVImport($file_name) {
		...
		if($this->showFieldNames) {
			...
			$i=0;
			foreach($this->xml_tags as $x){ 
			    $this->xml_tags[$i] = $this->remove_utf8_bom(trim($x)); 
			    $i++;
			}
...
//_print_r($this->required_fields);
			...
			// set date as current if not set
			if($field_name=="date_added" && !$val) $val = date("Y-m-d H:i:s");
			...
			if($this->data != "ad" || ($this->data=="ad" && $field_name != "pictures" && $field_name != "mgm_email" && $field_name != "date_added")) {
				if($i) $sql_str.=", ";
		...
		if($this->data == "ad") {
		if((!isset($parsed_array['date_added']) || !$parsed_array['date_added']) && (!isset($this->default['date_added']) || !$this->default['date_added'])) {
			if(!isset($date_added) || !$date_added) {

				$date_added = date("Y-m-d H:i:s");
			}
		}
			$sql_str.=", date_added = '$date_added', date_updated = '$date_added'";
			$pkg = 0;
...
/*		else { // check if the template has a category, if not ask to choose one
	...
	function getQueryCondition() {
		return $this->where_cond;
	}
	function setQueryCondition($sql) {
		$this->where_cond = $sql;
	}
	...
	function exportListings($type){
		...
		//echo "query condition: ".$where."\n";
		
		if(!$where) $where_start = " where"; else $where_start = " and";
		
		$where.=$where_start." ".TABLE_CATEGORIES."_lang.lang_id = '$crt_lang' and ".TABLE_PACKAGES."_lang.lang_id='$crt_lang' ";
		$where_start = " and ";
		...
		$sql = "select ".TABLE_ADS.".*, ".TABLE_ADS.".id as id, date_format(`date_added`,'$date_format') as date_formatted, date_format(`date_expires`,'$date_format') as date_expires_formatted, (date_expires<'$timestamp' and date_expires is not null and ".TABLE_ADS.".active=0) as expired, ".TABLE_CATEGORIES."_lang.name as category, ".TABLE_PACKAGES."_lang.name as plan, ".TABLE_PACKAGES.".amount as plan_amount, ".TABLE_USERS.".username as username, ".TABLE_USERS_PACKAGES.".pending as pending_package from ".TABLE_ADS." 
