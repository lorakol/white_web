...
class packages {
	...
	static function getHtmlEditor($id) {
		global $db;
		$editor=$db->fetchRow('select `html_editor` from '.TABLE_PACKAGES.' where `id`="'.$id.'"');
		return $editor;
	}
	...
	static function getAllForm($group='') {
		...
		$where_str = "";
		if($group!=0)  $where_str .= " and ( ".TABLE_PACKAGES.".`groups` REGEXP CONCAT('(^|,)(', REPLACE('$group', ',', '|'), ')(,|$)') or `groups` = 0 ) ";
	...
	function getAllPlans($group='', $category='', $user_id = '') {
		...
		if($category)  $where_str .= " and ( ".TABLE_PACKAGES.".`categories` REGEXP CONCAT('(^|,)(', REPLACE('$category', ',', '|'), ')(,|$)') or `categories` = 0 ) ";
		if($group!=0)  $where_str .= " and ( ".TABLE_PACKAGES.".`groups` REGEXP CONCAT('(^|,)(', REPLACE('$group', ',', '|'), ')(,|$)') or `groups` = 0 ) ";
