...
function getSMSGateway($code) {
	...
	function getGeneralSettings() {
		...
		$settings = $db->fetchAssoc("select * from ".$config_table_prefix."sms_gateways_settings");
		$settings['sms_content'] = cleanStr($settings['sms_content']);
		return $settings;
	...
	function editGeneralSettings() {
		...
		$sms_content = escape($_POST['sms_content']);
		$db->query("update ".$config_table_prefix."sms_gateways_settings set `sms_content` = '$sms_content', `type`='$type'");
		return $settings;
