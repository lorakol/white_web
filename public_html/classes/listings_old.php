...
//		$start = rand(0,$t);
		...
		$result=$this->getShortListings($where,"","",0,0);
		shuffle ( $result );
		$result = array_slice($result, 0, $no_featured);
		return $result;
	...
	function getListing($id, $not_formatted=0, $result= '') {
		...
		if(!$result) {
			...
			$sql = "select ".TABLE_ADS.".*$mlang_vars, date_format(".TABLE_ADS.".`date_added`,'$date_format') as date_nice, date_format(".TABLE_ADS.".`date_updated`,'$date_format') as date_updated_nice, date_format(".TABLE_ADS.".`date_expires`,'$date_format') as date_expires_nice, UNIX_TIMESTAMP(".TABLE_ADS.".`date_added`) as `time_added`, UNIX_TIMESTAMP(".TABLE_ADS.".`date_updated`) as `time_updated`, (".TABLE_ADS.".`date_expires`<'$timestamp' and ".TABLE_ADS.".`date_expires`!='0000-00-00 00:00:00' and ".TABLE_ADS.".active=0) as expired from ".TABLE_ADS." 
			where ".TABLE_ADS.".`id`='$id'";
...
/*	$locations_str="";
	...
	$where_not_featured = $where;

	$where_featured = " where class_ads.active=1 and ".TABLE_ADS.".featured>0 ";

	$featured = $this->getShortListings($where_featured,"","",0,0);

	shuffle ( $featured );
	$featured = array_slice($featured, 0, 7);

	$where_not_featured .= " and ".TABLE_ADS.".featured <= 0 ";
	$not_featured = $this->getShortListings($where_not_featured,$order_by,$order_way,$start,$ads_per_page, $keyword, $str_zip, $join_zip, $join_cat);

	return array_merge($featured,$not_featured);
