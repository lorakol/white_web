...
class auth {
	...
	function haslogin() {
		...
		if(!isset($_POST[$this->post_index['auth_name']]) || !isset($_POST[$this->post_index['pass']])) return 0;
	...
	function savelogin($ip, $uid=null, $provider=null, $step=0) {

		global $db;
		
		$timestamp = date("Y-m-d H:i:s");
		// if step 2 authentication
		if($step==2) {
			global $config_abs_path;
			require_once $config_abs_path."/classes/users.php";
			$user_id = users::getUserIdByAuthName($this->auth_name);
			$user = users::getUserInfo($user_id);
			$this->passhash = $user['password'];
			$res=$db->query("delete from ".TABLE_LOGIN_HISTORY." where auth_name='$this->auth_name' and succeeded == 2");
		}	
		...
		$res=$db->query("update ".TABLE_USERS." set last_login = '$timestamp' where `$field`='$this->auth_name'");
	...
	function saveLoginFirstStep($ip, $uid=null, $provider=null) {

		global $db;
		
		if($provider) $this->auth_name = $uid;
		$timestamp = date("Y-m-d H:i:s");
		
		$ga = new PHPGangsta_GoogleAuthenticator();
		$user_id = users::getUserIdByAuthName($this->auth_name);
		$user = users::getUserInfo($user_id);
		$otp = $ga->getCode($user['2fa_secret']);
			
		$res=$db->query("insert into ".TABLE_LOGIN_HISTORY." set auth_name='$this->auth_name', date_login = '$timestamp', ip = '$ip', succeeded = 2, `otp`='$otp'");
		$last_id = $db->insertId();
		
		// send SMS if configured
		$security_settings  = common::getCachedObject("base_security");
		if($security_settings['2fa_enable_sms']) {
		
			// check if a phone no exists
			$phone_no = $user[$security_settings['2fa_phone_field']];
			if(!$phone_no) return 0;

			$this->sendSMSOTP($phone_no, $otp, $last_id);

		} // if sms
		
	}

	function sendSMSOTP($phone_no, $otp, $last_id) {


		// check if a SMS gateway is configured
		global $config_abs_path;
		...
		$gcl = new $class_name;
		global $lng;
		$sent = $gcl->send($phone_no, $lng['login']['your_otp_sms'].$otp, $last_id, "otp");
	...
	function admin_checklogin($aname=null,$passhash=null) {
		...
		if( ($settings["admin_username"]==$aname || $settings["admin_email"]==$aname) && $admin_password==$passhash) { 
			if($security_settings['admin_enable_2fa'] && $security_settings['admin_2fa_secret'])
				return 2; // two factor authentication
			return 1;
		}	
		return 0;
	}

	function admin_savelogin($ip, $step=0) {
		global $db;
		...
		if($step==2) {
			...
			global $config_abs_path;
			require_once $config_abs_path."/classes/settings.php";
			$this->passhash = $settings["admin_password"];
			$res=$db->query("delete from ".TABLE_LOGIN_HISTORY." where auth_name='$this->auth_name' and succeeded = 2");
		}	
		$timestamp = date("Y-m-d H:i:s");
		$res=$db->query("insert into ".TABLE_LOGIN_HISTORY." set auth_name = '$this->auth_name', date_login = '$timestamp', `ip` = '$ip', succeeded = 1");
	...
	function saveAdminLoginFirstStep($ip) {
		...
		$ga = new PHPGangsta_GoogleAuthenticator();
		$security_settings  = common::getCachedObject("base_security");
		$otp = $ga->getCode($security_settings['admin_2fa_secret']);
		...
		// send SMS if configured
		if($security_settings['admin_2fa_enable_sms']) {
		
			// check if a phone no exists
			$phone_no = $security_settings['admin_phone_number'];
			if(!$phone_no) return 0;
	...
	function isTwoFAEnabled($uid) {

		$usr=new users();
		$is_2fa = $usr->is2FA($this->auth_name);
		return $is_2fa;

	}

	function step2HasLogin($auth_name, $ip) {
	
		if(!isset($_POST['otp']) || !$_POST['otp']) { 
			global $lng;
			$this->setLoginError($lng['login']['error']['invalid_code']);
			return 0;
		}	

		// check if there is a registration in login_history and is not expired, with the same ip and otp
		$otp_post = escape($_POST['otp']);
		global $db;
		$exists = $db->fetchRow("select id from ".TABLE_LOGIN_HISTORY." where `auth_name` like '{$this->auth_name}' and `ip` like '$ip' and succeeded=2");
		if(!$exists){
			global $lng;
			$this->setLoginError($lng['login']['error']['2fa_unknown_error']);
			return 0;
		}
		...
		if($auth_name == $settings['admin_username']) {
			$security_settings  = common::getCachedObject("base_security");
			$secret = $security_settings['admin_2fa_secret'];
		}
		else {
			if($settings['enable_username']) $field="username"; else $field="email";
			$secret = $db->fetchRow("select `2fa_secret` from ".TABLE_USERS." where `$field` like '$auth_name'");
		}	

		$ga = new PHPGangsta_GoogleAuthenticator();
		$valid = $ga->verifyCode($secret, $otp_post, 4);    // 4 = 4*30sec clock tolerance
