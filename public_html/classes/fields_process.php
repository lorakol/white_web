...
//		$f = new fields($this->type);
		if($this->type=="cf") { 
			...
			$fields=common::getCachedObject("base_listing_fields", array("fieldset" => $set, "group"=>$gr));
			}
		else { 
			$fields=common::getCachedObject("base_user_fields", array("group" => $set));
		}
	...
	function validate_field($field, $fieldset) {
		...
		if(!$val) {
			...
			if($validation_type!='' && $val) {
				...
				case "price":
					if(!validator::valid_price($val, $price_point, $price_th_separator)) $err=1;
				break;
		...
		if($unique && $val && !$this->validateUniqueField($caption, $val)) {
	...
	function add_fields($set, $array_format=0, $group=0) {

		// $array_format = 1 -> pending edited

		global $db;
		global $default_fields_types;
		$has_admin_verification = 0;
		...
		for ($i=0; $i<$no_fields; $i++)	{
			...
			if($this->type=="uf")
				$admin_verification = $fields[$i]['admin_verification'];
			...
			elseif($field_type=="google_maps" && $this->type=="cf"){ // for distance search
				...
				if(isset($_POST[$caption])) {
					...
					if($field_type=="textarea") $field_val = str_replace("\n", "\n<br />", escapeHtml($field_val));
					...
					if($field_type=="htmlarea") {
						...
						$field_val = strip_tags($field_val, $tags_list);
			...
			if(!$this->edit || $field_val || ($this->edit && $field_type!="image" && $field_type!="logo" && $field_type!="file" && $field_type!="video" && $field_type!="audio")) {
				...
				if(!$array_format)  {
					...
					if($field_val || ($field_type=="checkbox" && $field_val==0)) {
						if(!$admin_verification || ($is_admin || $is_mod))
							$sql.=", `$caption` = '".$field_val."'";
						else {
							$sql.=", `temp_$caption` = '".$field_val."'";
							$has_admin_verification = 1;
						}
					}
		...
		if($this->type=="uf" && $fields[$i]["public"]==2) {
		  $array_uc = array("textbox","textarea","htmlarea","menu","url","email","user_email","phone","whatsapp","date","button");
		...
		if($has_admin_verification) {
			$sql.=", `admin_verification` = 1";
		}
...
//echo $sql; exit(0);
	...
	function acceptAdminVerification($id, $caption) {

		global $db;

		// textbox, textarea, htmlarea, url, twitter, file, image, logo, youtube, video, audio
		if($this->type=="uf") $table=TABLE_USERS; else $table=TABLE_ADS;

		// copy the value
		$db->query("update $table set `$caption`=`temp_{$caption}` where id='$id'");
		//echo "update $table set `$caption`=`temp_{$caption}` where id='$id'";
		// remove temp value
		$db->query("update $table set `temp_{$caption}`=null where id='$id'");
//echo "update $table set `temp_{$caption}`=null where id='$id'";

		// remove admin_verification flag
		if(!$this->hasAdminVerification($id)) $db->query("update $table set `admin_verification`=0 where id='$id'");

	} // end acceptAdminVerification

	function denyAdminVerification($id, $caption) {
	...
	} // end denyAdminVerification

	function hasAdminVerification($id) {

		global $db;
		$fields = $this->getAdminVerificationFields();
		if($this->type=="uf") $table=TABLE_USERS; else $table=TABLE_ADS;
		foreach($fields as $f) {

			$val = $db->fetchRow("select `$f` from $table where `id` ='$id'");
	...
	function getAdminVerificationFields() {

		global $db;
		$fields = $db->fetchRowList("select `caption` from ".$this->table." where `active`==1 and `admin_verification`==1 ");
		return $fields;
