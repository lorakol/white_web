...
class categories {
	...
	static function getDefaultName($id) {

		global $db;
		if(!is_numeric($id)) return;
		$lang = languages::getDefault();
		$name=$db->fetchRow("select `name` from ".TABLE_CATEGORIES."_lang where id='$id' and `lang_id` = '$lang'");
 		return $name;
	}
	...
	function getNo() {
		...
		global $db;
		$no=$db->fetchRow('select count(*) from '.TABLE_CATEGORIES." where `active`=1");
		return $no;
	}
	...
	function getAllOptions ($crt_group=0, $parent_id=0) {
		...
		if($crt_group) {
			if($crt_group==-1) // nologin user
				...
				$group_regexp = " and (`groups` REGEXP CONCAT('(^|,)(', REPLACE('$crt_group', ',', '|'), ')(,|$)') or `groups`=0 )";
		}
		...
		$result=$db->fetchAssocList("select ".TABLE_CATEGORIES.".`id`, `name`, description, `parent_id`, `groups`, `order_no`, `level`, `slug`, `picture`, `icon`
