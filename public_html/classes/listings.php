...
class listings {
	...
	function isExpired($id=0) {
		...
		$res=$db->query("select count(*) from ".TABLE_ADS." where `id`='$id' and date_expires<'$timestamp' and date_expires!=null ");
	...
	function renew($id, $is_admin=1) {
		...
		if($is_admin) {
			$this->incCat($id);
			$res=$db->query("update ".TABLE_ADS." set `active`=1, user_approved=1, date_added='$timestamp', date_updated='$timestamp', $sql_exp where `id`='$id'");
		} else {
			$res=$db->query("update ".TABLE_ADS." set date_added='$timestamp', date_updated='$timestamp', $sql_exp where `id`='$id'");
		}
	...
	function renewOptions($id) {
		...
		$db->query("update ".TABLE_ADS." set `featured`='$featured', `highlited`='$highlited', `priority`='$priority', `urgent`='$urgent', `video`='$video', `site_url`='$url' where id='$id'");
	...
	function getNoRecent($sql_where='') {
		...
		if($days_recent>0)
			$sql = "select count(*) from ".TABLE_ADS." where active=1 and `date_updated` > '$date_x_days_before_start' ".$locations_str;
	...
	function getStoreListings($page,$ads_per_page,$order_by,$order_way,$crt_usr='') {
		...
		if(!$order_by) $order_by='date_updated';
	...
	function getRecent($page, $ads_per_page, $order_by, $order_way, $sql_where = '') {
		...
		if($days_recent>0) {
			$where="where `date_updated` > '$date_x_days_before_start' and ".TABLE_ADS.".active =1".$locations_str;
		}
		else {
...
*/
	...
	function getLatest($no_latest, $where = '') {
		...
		$order_by_str="order by date_updated desc";
		$order_way='';
		$start=0;
	...
	function getListing($id, $not_formatted=0, $result= '') {
		...
		if(!$result) {
			...
			$sql = "select ".TABLE_ADS.".*$mlang_vars, date_format(".TABLE_ADS.".`date_added`,'$date_format') as date_nice, date_format(".TABLE_ADS.".`date_updated`,'$date_format') as date_updated_nice, date_format(".TABLE_ADS.".`date_expires`,'$date_format') as date_expires_nice, UNIX_TIMESTAMP(".TABLE_ADS.".`date_added`) as `time_added`, UNIX_TIMESTAMP(".TABLE_ADS.".`date_updated`) as `time_updated`, (".TABLE_ADS.".`date_expires`<'$timestamp' and ".TABLE_ADS.".`date_expires`!=null and ".TABLE_ADS.".active=0) as expired from ".TABLE_ADS." 
			where ".TABLE_ADS.".`id`='$id'";
...
//_print_r($result);
	...
	if($result['site_url']) {
	    if(!$this::urlEnabled($id)) $result['site_url'] = '';
	}
...
//_print_r($result);
		...
		// x days ago
		if(!$admin_side && !$result) { 
			$result['date_nice'] = $this->makeTimeAgoDate($result['time_added'], $result['date_nice']);
			$result['date_updated_nice'] = $this->makeTimeAgoDate($result['time_updated'], $result['date_updated_nice']);
		}
		...
		if($ads_settings['enable_auctions']) {
			...
			global $crt_usr;
			if( $result['user_id'] == $crt_usr && isset($result['auction']['no_bids']) && $result['auction']['no_bids'] ) {
	...
	function getNoShortListings($where, $q_join='', $noloc = 0){
		...
		if(strstr($where, TABLE_CATEGORIES."_lang")) {
			global $crt_lang;
			$join .= "inner join ".TABLE_CATEGORIES."_lang on (".TABLE_ADS.".category_id=".TABLE_CATEGORIES."_lang.id and ".TABLE_CATEGORIES."_lang.lang_id ='$crt_lang' ) ";
		}
...
//		$where.=" and EXISTS (select id from ".TABLE_ADS_PICTURES." where ad_id=".TABLE_ADS.".id)";
//			$join_pictures = "inner join ".TABLE_ADS_PICTURES." on ".TABLE_ADS.".id=".TABLE_ADS_PICTURES.".ad_id ";
		...
		$sql = "select ".TABLE_ADS.".*$mlang_vars, ".TABLE_ADS.".id as adid, date_format(".TABLE_ADS.".`date_added`,'$date_format') as date_nice, date_format(".TABLE_ADS.".`date_updated`,'$date_format') as date_updated_nice, date_format(".TABLE_ADS.".`date_expires`,'$date_format') as date_expires_nice, UNIX_TIMESTAMP(".TABLE_ADS.".`date_added`) as `time_added`, UNIX_TIMESTAMP(".TABLE_ADS.".`date_updated`) as `time_updated`, (".TABLE_ADS.".date_expires < '$timestamp' and ".TABLE_ADS.".date_expires!=null and ".TABLE_ADS.".active=0) as expired
		$str_slugs
		$q_vars 
...
//_print_r($arr);
		...
		foreach ($arr as $row) {
			...
			$result[$i]['date_updated_nice'] = $this->makeTimeAgoDate($result[$i]['time_updated'], $result[$i]['date_updated_nice']);
...
function getAllId($limit=0) {
	...
	$array = $db->fetchRowList("select id from ".TABLE_ADS." where active=1 order by date_updated desc".$str_limit);
	return $array;
...
function getAllSitemapQuery($limit=0) {
	...
	return "select id, $title_var $subdomain_str from ".TABLE_ADS." where active=1 order by date_updated desc".$str_limit;
...
// ------------------- favourites -----------------------
	...
	static function getLastListings($no) {
		...
		$sql = "select ".TABLE_ADS.".id, ".TABLE_ADS.".$title_var as `title`, ".TABLE_ADS.".user_id, date_format(`date_updated`,'$date_format') as `date`, ".TABLE_USERS.".username as user, ".TABLE_USERS.".email, ".TABLE_ADS_EXTENSION.".mgm_email  from ".TABLE_ADS." left join ".TABLE_USERS." on ".TABLE_ADS.".user_id=".TABLE_USERS.".id left join ".TABLE_ADS_EXTENSION." on ".TABLE_ADS.".id = ".TABLE_ADS_EXTENSION.".id order by date_updated desc limit ".$no;
		...
		foreach($post_array as $key=>$value) {
			...
			switch($key) {
				...
				case "expired": 
					if($found) $where.=" and "; else $where = " where ";
					$timestamp = date("Y-m-d H:i:s");
					$where .= " (".TABLE_ADS.".`active`=0 and ".TABLE_ADS.".`date_expires` < '$timestamp' and ".TABLE_ADS.".`date_expires`!=null) ";
					$found = 1;
				break;
	...
	static function getNoExpiredListings($crt_usr='') {
		...
		$sql="select count(id) from ".TABLE_ADS." where active=0 and date_expires < '$timestamp' and date_expires!=null and date_expires is not null";
		if($crt_usr) $sql.=' and user_id='.$crt_usr;
		$no=$db->fetchRow($sql);
		return $no;
	}
	...
	function getListingsDetailed($where,$order_by='',$order_way='',$general_row='',$ads_per_page=''){ 
		...
		$sql = "select ".TABLE_ADS.".*$mlang_vars, ".TABLE_ADS.".id as adid, ".TABLE_ADS_PICTURES.".picture, ".TABLE_ADS_PICTURES.".folder, ".TABLE_ADS_PICTURES.".`id` as `picture_id`, date_format(".TABLE_ADS.".`date_added`,'$date_format') as date_nice, date_format(".TABLE_ADS.".`date_updated`,'$date_format') as date_updated_nice, date_format(".TABLE_ADS.".`date_expires`,'$date_format') as date_expires_nice, UNIX_TIMESTAMP(".TABLE_ADS.".`date_added`) as `time_added`, UNIX_TIMESTAMP(".TABLE_ADS.".`date_updated`) as `time_updated`, (".TABLE_ADS.".date_expires<'$timestamp' and ".TABLE_ADS.".date_expires!=null and ".TABLE_ADS.".active=0) as expired $str_slugs
		from ".TABLE_ADS." 
...
//_print_r($arr);
		...
		foreach ($arr as $row) {
			...
			foreach($result[$i]['pending_actions'] as $action) {
				...
				else if($action['type'] == "auto_repost") $result[$i]['pending_info'].=$lng['listings']['auto_repost'].'<br />';
			}

			
			// not finished payments
			// get unique invoices
			// make an info string for each invoice
			$result[$i]['incomplete_payments'] = array();
			$invoices = $db->fetchAssocList("SELECT DISTINCT `".TABLE_ACTIONS."`.invoice, ".TABLE_ACTIONS.".date from `".TABLE_ACTIONS."` left join `".TABLE_PAYMENT_ACTIONS."` on `".TABLE_PAYMENT_ACTIONS."`.id = `".TABLE_ACTIONS."`.`invoice` where `".TABLE_PAYMENT_ACTIONS."`.completed=0 and ((`object_id` = ".$row['id']." and (`type` like 'featured' or `type` like 'highlited' or `type` like 'priority' or `type` like 'urgent' or `type` like 'video' or `type` like 'url' or `type` like 'newad' or `type` like 'renewad' or `type` like 'bump' or `type` like 'auto_repost')) or (`object_id` = '".$row['usr_pkg']."' and (`type` like 'newpkg' or `type` like 'renewpkg') )) order by `invoice`  desc");
			$j=0;
			
			foreach($invoices as $inv) {
				$invoice = $inv['invoice'];
				$result[$i]['incomplete_payments'][$j]['invoice'] = $invoice;
				
				$action_str = $db->fetchRow("select `action` from ".TABLE_PAYMENT_ACTIONS." where id='$invoice'");
// 				echo ($action_str);
				//echo $pa->actionsStr($action_str);
				$result[$i]['incomplete_payments'][$j]['action_str'] = $pa->actionsStr($action_str);
				$j++;
			...
			if(!$result[$i]['user_id']) {
				...
				} else 	$result[$i]['username'] = $result[$i]['email'] = $result[$i]['mgm_name']==$result[$i]['ip']='';
			}

			// check if ip is blocked
			if($result[$i]['ip'] && $admin_side) { 
				$result[$i]['blocked'] = blocked_ips::isBlocked($result[$i]['ip']);
				$result[$i]['flagged'] = flagged_ips::isFlagged($result[$i]['ip']);
			}
	...
	function getDateExpires($id) {
		...
		$expires = $db->fetchAssoc("select `date_expires`, date_format(`date_expires`,'%e') as `day`, date_format(`date_expires`,'%c') as `month`, date_format(`date_expires`,'%Y') as `year`, (`date_expires`<'$timestamp' and `date_expires`!=null and active=0) as expired  from ".TABLE_ADS." where `id`=$id");
...
function getAdvSearch($post_array,$page, $ads_per_page, $extra_where='') {
	...
	if(in_array("category_name", $fields)) $join_cat = " inner join ".TABLE_CATEGORIES."_lang on (".TABLE_CATEGORIES."_lang.id = ".TABLE_ADS.".category_id and ".TABLE_CATEGORIES."_lang.lang_id='$crt_lang' ) ";
	...
	foreach ($post_array as $key=>$val) {
		...
		switch($key) {
				...
				if($ads_settings['search_type']=="exact") {
					...
					foreach($fields as $f) {
						...
						if($mbyte_characters_search)
							...
							$str.=" lower(".$stable.".$f) REGEXP '".$db->getRegexStartWord()."".$val.$db->getRegexEndWord()."' ";
						$k++;
					}
						...
						for($i=$no_words; $i>0;$i--) {
							...
							if($w!='') {
								if($mbyte_characters_search)
									...
									$str.=" lower(".$stable.".$f) REGEXP '".$db->getRegexStartWord()."".$w.$db->getRegexEndWord()."' ";
								$j++;
							}
						}
						$k++;// no for fields
			...
			case "with_pic":
				...
				$str=" and EXISTS (select id from ".TABLE_ADS_PICTURES." where ad_id=".TABLE_ADS.".id)";
				$where.=$str;
				...
				if(isset($custom_fields[$crt_lang][$key]['search_type']) && $custom_fields[$crt_lang][$key]['search_type']=="interval") {
					...
					if(isset($custom_fields[$crt_lang][$key]['type']) && $custom_fields[$crt_lang][$key]['type'] && $no_lang>1 ) {
						...
						foreach($crt_lang_arr as $el) {
							if(in_arrayi($el, $val_array)) array_push($array_indexes, $i);
							$i++;
						}

						if(count($array_indexes)!=0) {
						...
						foreach($languages as $l) {
							...
							foreach($array_indexes as $idx) {
							
								$alt_val = $alt_lang_arr[$idx];

								if(!in_array(strtolower($alt_val), $arr_translations) && !in_arrayi($alt_val, $val_array)) array_push($arr_translations, strtolower($alt_val));
							}
						}
						...
						foreach($arr_translations as $tr) {
							...
							if($custom_fields[$crt_lang][$key]['search_type']=="keyword")
								...
								if(in_array($custom_fields[$crt_lang][$key]['type'], array("multiselect", "radio", "radio_group", "checkbox_group"))) {
									if($mbyte_characters_search)
									$q_translated.=" or ".TABLE_ADS.".".$key." like '%$tr%'";
									else
									$q_translated.=" or ".TABLE_ADS.".".$key." REGEXP '".$db->getRegexStartWord()."$tr".$db->getRegexEndWord()."'";
								}
								else
								$q_translated.=' or '.TABLE_ADS.'.'.$key.' like "'.$tr.'" ';
							}
...
//						if(in_array($custom_fields[$crt_lang][$key]['type'], array("multiselect", "radio", "radio_group")) && $custom_fields[$crt_lang][$key]['search_type']=="default") {
						if(in_array($custom_fields[$crt_lang][$key]['type'], array("multiselect", "radio_group", "checkbox_group")) && $custom_fields[$crt_lang][$key]['search_type']=="default") {

							if($mbyte_characters_search)
								$str.=" and (".TABLE_ADS.".".$key." like '%$val%' ".$q_translated.")";
							else
								$str.=" and (".TABLE_ADS.".".$key." REGEXP '".$db->getRegexStartWord()."$val".$db->getRegexEndWord()."' ".$q_translated.")";
...
//_print_r($where_array);
	...
	else $order_by=" order by $pri_ord date_updated";
...
/*	$locations_str="";
	...
	function Unflag($id) {
		...
		$res=$db->query('delete from '.TABLE_FLAGGED_IPS.' where ip like "'.$ip.'"');
...
// alternative function
	...
	*/
	...
	function setEmailOn($id) {
	
		global $ads_settings;
		global $crt_usr;
		if(!$this->belongsToUser($id,$crt_usr)) return;
		global $db;
		$db->query("update ".TABLE_ADS." set `email_contact`=1 where id='$id'");
		return 1;
	
	}

	function setEmailOff($id) {
	
		global $ads_settings;
		global $crt_usr;
		if(!$this->belongsToUser($id,$crt_usr)) return;
		global $db;
		$db->query("update ".TABLE_ADS." set `email_contact`=0 where id='$id'");
		return 1;
	...
	global $db;
	$res=$db->query("update ".TABLE_ADS." set `featured`=0 where `id`='$id'");
	return 1;
    }
	...
	global $db;
	$res=$db->query("update ".TABLE_ADS." set `highlited`=0 where `id`='$id'");
	return 1;
	...
	global $db;
	$res=$db->query("update ".TABLE_ADS." set `urgent`=0 where `id`='$id'");
	return 1;
