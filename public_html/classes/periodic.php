...
function deleteExpiredAds() {
	...
	if(!$days_del_expired) $sql="select id from ".TABLE_ADS." where `active`=0 and `date_expires` <= '$timestamp' and date_expires is not null";
	// delete after a time
	else $sql = "select id from ".TABLE_ADS." where active=0 and date_expires <= '$timestamp' and date_expires is not null and date_add(`date_expires`, interval '$days_del_expired' day) <= '$timestamp'";
...
function markExpiredAds() {
	...
	where ".TABLE_ADS.".active=1 and date_expires <= '$timestamp' and date_expires is not null;");
...
// and date_added!=date_add(date_added, interval no_days day)
	...
	$result_del=$db->query("delete from ".TABLE_OPTIONS." where date_expires <= '$timestamp' and `date_expires` is not null;");
	...
	$result=$db->query("select * from ".TABLE_OPTIONS." where `option` like 'store' and date_expires <= '$timestamp' and `date_expires` is not null order by `object_id`;");
	...
	$result=$db->query("select * from ".TABLE_OPTIONS." where `option` not like 'store' and date_expires <= '$timestamp' and `date_expires` is not null order by `object_id`;");
...
function deleteExpiredUsrPkg($days) {
	...
	if(!$days) $sql="select id from ".TABLE_USERS_PACKAGES." where `active`=0 and fixed=0 and `date_end` <= '$timestamp' and date_end is not null";

	else $sql = "select id from ".TABLE_USERS_PACKAGES." where active=0 and fixed=0 and date_end <= '$timestamp' and date_end is not null and date_add(`date_end`, interval '$days' day) <= '$timestamp'";
...
function markExpiredUsrPkg() {
	...
	$result=$db->query("select ".TABLE_USERS_PACKAGES.".id, user_id, ads_left, date_end is not null as expired, ".TABLE_USERS.".`username`, ".TABLE_USERS.".`email`, ".TABLE_USERS.".`{$settings['contact_name_field']}`,".TABLE_PACKAGES.".`type` from ".TABLE_USERS_PACKAGES."
	...
	where ".TABLE_PACKAGES.".`type` like 'sub' and ".TABLE_USERS_PACKAGES.".active=1 and (( date_end <= '$timestamp' and date_end is not null) or `ads_left` = 0 ) ;");
	while($row = $db->fetchAssocRes($result)) {
		$id = $row['id'];
		$user_id = $row['user_id'];
...
function notifyExpiredUsrPkg() {
	...
	left join ".TABLE_USERS." on ".TABLE_USERS_PACKAGES.".user_id=".TABLE_USERS.".id
	left join ".TABLE_PACKAGES." on ".TABLE_USERS_PACKAGES.".package_id=".TABLE_PACKAGES.".id
	where ".TABLE_USERS_PACKAGES.".active=1 and date_end is not null and date_end between '$date_x_days_from_now_start' and '$date_x_days_from_now_end';");
	...
	function periodicSitemap() {
		...
		$val = $db->fetchAssoc("select (generated_last is null or date_add(generated_last, interval $days day)<CURDATE()) as go from ".TABLE_SITEMAP);
		if($val['go']) {
