...
class banners {
	...
	function getTemplateBanners($pos, $section, $categ, $bloc='') {
		...
		if($categ && is_numeric($categ)) $cat_regexp = " or categories REGEXP CONCAT('(^|,)(', REPLACE('$categ', ',', '|'), ')(,|$)')";
		$cat_cond = "and ( categories like '0' $cat_regexp )";
		}
		if($section) {
			$section = cleanStr($section);
			$section_cond = "and ( sections like '0' or sections REGEXP CONCAT('(^|,)(', REPLACE('$section', ',', '|'), ')(,|$)') )";
		}
		...
		$sql = "select * from ".TABLE_BANNERS." where `position`='$pos'".$str_impressions." and ( `max_clicks` = 0 or `clicks` < `max_clicks` ) and ( date_start <= '$timestamp' or date_start is null) and ( date_end >='$timestamp' or date_end is null) $cat_cond $bloc_cond $section_cond order by rand() limit $no_banners";
		...
		if(in_array("banners_location", $modules_array) && ($self_noext=="listings" || $self_noext=="details")) {
			...
			if($diff>0 && $bloc_settings['display_any']) {
				...
				$sql = "select * from ".TABLE_BANNERS." where `position`='$pos'".$str_impressions." and ( `max_clicks` = 0 or `clicks` < `max_clicks` ) and ( date_start is null or date_start <= '$timestamp' or date_start is null) and ( date_end >='$timestamp' or date_end is null) $cat_cond $bloc_cond $section_cond $str_found order by rand() limit $diff";
