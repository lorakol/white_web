...
function uploadUrl() {
	...
	if($this->resize) {
		...
		if($this->watermark) {
		 ...
		 {
		 if(!$this->cURLCopy($this->file_path, $this->input))
		 {
...
function verifyUrl(){
	...
	if(_isCurl()) {
	    if(!$this->cURLCopy($this->file_path, $this->input)) $this->error.=$lng['ie']['errors']['could_not_save_file'].$this->input;
	} else
	{
	if(!copy($this->input, $this->file_path)) {
		if(!$this->copyemz($this->input, $this->file_path)) {
			$this->error.=$lng['ie']['errors']['could_not_save_file'].$this->input;
			echo $lng['ie']['errors']['could_not_save_file'].$this->input;
			return 0;
		}
	}
	}
...
function cURLCopy($file_path, $url) {
...
//	curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
    ...
    if($info['size_download']==0) return 0;
    return 1;
}
