...
//***
	...
	function getTable($table, $set='') {
		...
		if(!$set) {
			...
			$where_str = " where ( `set_id` REGEXP CONCAT('(^|,)(', REPLACE('$set', ',', '|'), ')(,|$)') or `set_id` = 0 ) and `lang_id` = '$crt_lang' ";
		}
	...
	function getElements($table, $lang_id, $set='') {
		...
		if(!$set) {
			...
			$where_str = " where ( `dep` REGEXP CONCAT('(^|,)(', REPLACE('$set', ',', '|'), ')(,|$)') or `dep` = 0 ) and `lang_id` = '".escape($lang_id)."' ";
		}
		global $order_dep_fields;
		if($order_dep_fields) $order_by="name";
		else $order_by="id";
