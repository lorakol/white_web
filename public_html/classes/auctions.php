...
class auctions {
	...
	function addBid($aid, $ad_id, $bid, $message, $currency='') {
		...
		if($ads_settings['notify_when_new_bid']) {
			...
			$usr = new users;
			$user_id = listings::getUser($ad_id);
			$to_details = $usr->getContactData($user_id);
			...
			$sender_email = cleanStr(users::getEmail($crt_usr));

			$array_subject = array("ad_id" => $ad_id);

			$array_message = array("ad_id" => $ad_id, "ad_title" => $ad_title, "message" => nl2br($message), "sender_name" => $sender_name, "sender_email" => $sender_email, "contact_name" => cleanStr($to_details[$settings['contact_name_field']]), "bid"=>add_currency($bid, $currency));
			...
			if($settings['internal_messaging']) {
				...
				$msg = new messages();
				$from_phone="";
				$msg->add($user_id, $to_details['email'], $crt_usr, $sender_email, $from_phone, $ad_id, nl2br($message), '', 0);
