...
//		header("Content-Disposition: attachment; filename=".basename($fname));
	...
	function writeInserts($table) {
		...
		if(function_exists('mysqli_connect')) {
			$start_str = 'INSERT INTO ' . $table . ' VALUES ';
			$str = $start_str;
			$empty=1;
			while ($row = @mysqli_fetch_assoc ($res)) {
				...
				if($i>0) $str.=",".$this->newline;
				//$str .= 'INSERT INTO ' . $table . ' VALUES (' . $values . ');' . $this->newline;
				$str .= '(' . $values . ')';
				$empty=0;
				if($i==1000) { $str.=";".$this->newline; $this->writeSQL($str); $i=0; $str = $start_str; $empty=1; continue; }
				$i++;
...
*/
	...
	function executeBulk($query_str) {
		...
		global $config_demo;
		if($config_demo==1) return;
		global $config_table_prefix, $config_db_charset, $config_live_site;
		...
		foreach ($lines as $line) {
			...
			if(!empty($line) && $first!="-" && $first!="" && $first!="#" ){
				...
				$line = str_replace("##LIVE_SITE##", $config_live_site, $line);
	...
	function periodic() {
		...
		$val = $db->fetchAssoc("select (generated_last is null or date_add(generated_last, interval $days day)<date_add('$timestamp', interval 1 hour)) as go from ".TABLE_DB_BACKUP);
		if($val['go'])
		{
