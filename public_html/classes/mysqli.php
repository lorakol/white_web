...
class db_mysql
{
	...
	public function __construct($nodb = 0, $nocharset=0)
		...
		if (!$this->link)
		{
			...
			if($connect_error) $this->error .= @mysqli_connect_error();
			else $this->error="Could not connect to server: $config_db_server";
...
//			mysqli_query($this->link, "SET NAMES $mysql_names");
		...
		$version_str = @mysqli_get_server_info($this->link);
		$version_array = explode(".", $version_str);
		//_print_r($version_array);
		$this->set_version($version_array);
		
	}

	function set_version($version_array){
	
		$this->version = $version_array[0];
		$is_mariaDB = $this->isMariaDB($version_array[2]);
		if( $is_mariaDB || (!$is_mariaDB && $this->version<8)) { 
			$this->regex_start_word = "[[:<:]]";
			$this->regex_end_word = "[[:>:]]";
		}	
		else { 
			$this->regex_start_word = "\b";
			$this->regex_end_word = "\b";
		}	
	
	}
	
	function isMariaDB($str) {
	
		return stristr($str, "mariadb");
	...
	function getRegexStartWord() {
	
		return $this->regex_start_word;
		
	}
	function getRegexEndWord() {
	
		return $this->regex_end_word;
	...
	function set_charset($charset='', $collation='') {
		...
		if(!$charset && $config_db_charset) $charset = $config_db_charset;
		if(!$collation && $config_db_collation) $collation = $config_db_collation;
		...
		if( function_exists( 'mysqli_set_charset' ) )  {
			...
			if($collation) @mysqli_query($this->link, "COLLATE $collation");
		} 
		else {
	...
	function getError() {
		...
		if($config_debug) {
			...
			$error_query = $this->printErrorQuery();
			if($error_query) $str.="<br><span>SQL query/queries : </span>".$error_query;
			$str.="<br/><span>Error path:</span> ".$this->errorPath;
		}
		return $str;
