...
function getInvoiceActions($object_id, $type) {
	...
	if($type=="ad") $where_str .= " and ( `type` like 'newad' or `type` like 'renewad' or `type` like 'featured' or `type` like 'highlited' or `type` like 'priority' or `type` like 'video' or `type` like 'bump' or `type` like 'auto_repost' or `type` like 'urgent' or `type` like 'url' )";
	...
	foreach ($array as $arr) {
		...
		if ( $arr['type']=="newpkg" || $arr['type']=="renewpkg" ) {
			...
			$actions[$i]['name'] = $up->getPackageName($arr['object_id']);
				...
				// remove the last element from $actions
				array_pop($actions);
				continue;
			}
		}
		...
		if ( $arr['type']=="highlited" || $arr['type']=="video" || $arr['type']=="bump" || $arr['type']=="auto_repost" || $arr['type']=="urgent" || $arr['type']=="url" ) {
