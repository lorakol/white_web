...
class coupons {
	...
	static function getDiscountByCode($code, $user_id='', $group='', $type='') {
		...
		global $db;
		if($type && ($type=="ads" || $type=="store")) $str_type = " and `$type` = '1'"; else $str_type='';
		
		if($group) $str_group = " and ( `groups` REGEXP CONCAT('(^|,)(', REPLACE('$group', ',', '|'), ')(,|$)') or `groups` = -1 )";
		else $str_group = '';
		$timestamp = date("Y-m-d H:i:s");
		$arr=$db->fetchAssoc("select ".TABLE_COUPONS.".* from ".TABLE_COUPONS." where `code` like '$code'".$str_type.$str_group." and ( date_start <= '$timestamp' or date_start is null) and ( date_end >='$timestamp' or date_end is null) limit 1");
	...
	static function codeValid($code, $type, $user_id, $group) {
		...
		global $db;
		$timestamp = date("Y-m-d H:i:s");
		$arr = $db->fetchAssoc("select count(*) as no, ".TABLE_COUPONS.".* from ".TABLE_COUPONS." where `code` like '$code' and `$type` = 1 and ( date_start <= '$timestamp' or date_start is null) and ( date_end >='$timestamp' or date_end is null) and ( `groups` REGEXP CONCAT('(^|,)(', REPLACE('$group', ',', '|'), ')(,|$)') or `groups` = -1 ) group by `code`");
