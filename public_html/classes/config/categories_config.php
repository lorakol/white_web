...
class categories_config {
	...
	function add_multi() {
		...
		foreach ($languages as $lang) {
			...
			$lang_id = $lang['id'];
			$na = explode(",",$this->clean['name'][$lang_id]);

			$cat_name = $na[$i];

			$res=$db->query("insert into ".TABLE_CATEGORIES."_lang set `id` = '$id', `lang_id` = '$lang_id', `name` = '".$cat_name."', `description` = '".$this->clean['description'][$lang_id]."', `page_title` = '".$this->clean['page_title'][$lang_id]."', `meta_keywords` = '".$this->clean['meta_keywords'][$lang_id]."', `meta_description` = '".$this->clean['meta_description'][$lang_id]."';");
	...
	function copy($id) {
		...
		$array = $db->fetchAssoc("select * from ".$config_table_prefix."categories where `id`='$id'");
		$array_lang = $db->fetchAssocList("select * from ".$config_table_prefix."categories_lang where `id`='$id'");
		
		$db->query("insert into ".$config_table_prefix."categories set `picture`='{$array['picture']}', `icon`='{$array['icon']}', `parent_id`='{$array['parent_id']}', `fieldset`='{$array['fieldset']}', `groups`='{$array['groups']}', `level`='{$array['level']}', `active`='{$array['active']}', `order_no`='{$array['order_no']}'");
		
		$cat_id=$db->insertId();
		
		foreach($array_lang as $a) {
			if($a['lang_id']==$default_lang) $category_name = $a['name'];
			$db->query("insert into ".$config_table_prefix."categories_lang set `id`='$cat_id', `lang_id`='{$a['lang_id']}', `name`='".escape($a['name'])."', `description`='".escape($a['description'])."', `page_title`='".escape($a['page_title'])."', `meta_keywords`='".escape($a['meta_keywords'])."', `meta_description`='".escape($a['meta_description'])."'");
		...
		$slug = new slugs();
		$slug->addCategory($cat_id, $category_name);
