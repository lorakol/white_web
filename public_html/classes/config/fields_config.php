...
class fields_config {
	...
	function caption_exists($str, $tab='') {
		...
		// captions which start with temp_ are reserved for admin verification fields
		if (stripos($str, "temp_") === 0) return 1;
	...
	function check_form ($id='', $type='') {
		...
		if($this->getError()!='') {
			...
			if($this->type=="uf")
				array_push($c_array, "admin_verification");
	...
	function add() {
		...
		if($this->type=='uf') {
			...
			// admin_verification;
			$this->clean['admin_verification']=checkbox_value('admin_verification');
		}
		...
		if($this->type=='cf') {
			$fields_array = array("fieldset", "caption", "type", "order_no", "is_numeric", "validation_type", "size", "min", "max", "required", "editable", "advanced_search", "quick_search", "search_type", "max_uploaded_size", "extensions", "image_resize", "dep_id", "other_val", "all_val", "unique", "ext1", "groups");
			...
			$fields_array = array("caption", "type", "order_no", "is_numeric", "validation_type", "size", "min", "max", "required", "editable", "max_uploaded_size", "extensions", "image_resize", "dep_id", "groups", "other_val", "all_val", "public", "unique", "ext1", "admin_verification");
		...
		if($this->clean['type']!="depending" && $this->clean['type']!="terms" ) {
			...
			// create a duplicate field for admin verification
			if($this->type=='cf' && $this->clean['admin_verification'])
				$res_alter=$db->query("alter table `$alt_table` add `temp_".$this->clean['caption']."` ".$datatype);
	...
	function edit($id) {
		...
		//if($this->type=="uf") {
			...
			//else
			if($this->type=="uf" && $old_data['groups']==-1) $this->clean['groups']='-1';
		//}
		...
		if($this->type=='cf') {
			...
			$fields_array = array("fieldset", "validation_type", "size", "min", "max", "required", "editable", "advanced_search", "quick_search", "search_type", "max_uploaded_size", "extensions", "image_resize", "dep_id", "other_val", "all_val", "is_numeric", "active", "unique", "ext1", "groups");
			...
			$fields_array = array("validation_type", "size", "min", "max", "required", "editable", "public" , "max_uploaded_size", "extensions", "image_resize", "dep_id", "groups", "other_val", "all_val", "is_numeric", "active", "unique", "ext1", "admin_verification");
...
//echo $sql;
		...
		// add admin verification field or remove if needed
	if($this->type=='uf') {
	 if(!$old_data['admin_verification'] && $this->clean['admin_verification']) {

		 $field_name = "temp_".$old_data['caption'];
		 $ntype = "default null";
		 if($type=='textarea' || $type=='htmlarea' || $type=='checkbox_group' || $type=='youtube' || $type=='multiselect') $ntype ="null";

		 $fdesc = $db->fetchAssoc("describe ".TABLE_USERS." {$old_data['caption']}");
		 $ftype = $fdesc['Type'];
		 $db->query("alter table ".TABLE_USERS." add $field_name $ftype $ntype");
	 ...
	 if($old_data['admin_verification'] && !$this->clean['admin_verification']) {
		 // remove temp field
		 $field_name = "temp_".$old_data['caption'];
		 $db->query("alter table ".TABLE_USERS." drop $field_name ");
	 }
 } // end if uf

		$this->clearFieldsCache();

		do_action("end_edit_custom_field", array($this->type, $id, $old_data['caption']));
	...
	function moveUp($order_no, $fset = '') {
		...
		if(!$fset) {
			...
			$goto = $db->fetchRow("select `order_no` from `".$this->table."` where ( fieldset REGEXP CONCAT('(^|,)(', REPLACE('$fset', ',', '|'), ')(,|$)')  or fieldset = 0 ) and `order_no` < $order_no order by order_no desc limit 1");
			if(!$goto) return 0;
	...
	function moveDown($order_no, $fset = '') {
		...
		if(!$fset) {
			...
			$goto = $db->fetchRow("select `order_no` from `".$this->table."` where ( fieldset REGEXP CONCAT('(^|,)(', REPLACE('$fset', ',', '|'), ')(,|$)')  or fieldset = 0 ) and `order_no` > $order_no order by order_no asc limit 1");
			if(!$goto) return 0;
	...
	function reArrange() {
		...
		if($fset) {
			if($fset>0)
				$where_str = " and (`$f` REGEXP CONCAT('(^|,)(', REPLACE('$fset', ',', '|'), ')(,|$)') or `$f` = 0)";
