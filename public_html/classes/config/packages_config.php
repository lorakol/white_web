...
class packages_config {
	...
	function Activate($id='') {
		...
		$res=$db->query('update '.TABLE_ADS.' set active=1 where `user_approved`=1 and `package_id`="'.$id.'" and (`date_expires`>"'.$timestamp.'" or `date_expires` is null)');
...
// allow 0 images
		...
		if($this->getError()!='')
		{
			...
			$this->tmp['html_editor'] = checkbox_value("html_editor");
	...
	function add() {
		...
		$this->clean['html_editor'] = checkbox_value("html_editor");
		...
		$res=$db->query('insert into '.TABLE_PACKAGES.' (`type`, `amount`, `no_words`, `no_days`, `no_pictures`, `photo_mandatory`, `no_ads`, `subscription_time`, `groups`, `categories`, `order_no`, `featured`, `highlited`, `priority`, `video`, `urgent`, `url`, `allow`, `html_editor`) values ("'.$this->clean['type'].'", "'.$this->clean['amount'].'", "'.$this->clean['no_words'].'", "'.$this->clean['no_days'].'", "'.$this->clean['no_pictures'].'", "'.$this->clean['photo_mandatory'].'", "'.$this->clean['no_ads'].'", "'.$this->clean['subscription_time'].'", "'.$this->clean['groups'].'", "'.$this->clean['categories'].'", "'.$order_no.'", "'.$this->clean['featured'].'", "'.$this->clean['highlited'].'", "'.$this->clean['priority'].'", "'.$this->clean['video'].'", "'.$this->clean['urgent'].'", "'.$this->clean['url'].'", "'.$this->clean['allow'].'", "'.$this->clean['html_editor'].'");');
