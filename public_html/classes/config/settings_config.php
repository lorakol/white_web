...
class settings_config {
	...
	function check_form_ads_settings() {
		...
		if($this->getError()!='')
		{
			...
			$array_inputs_null = array('thmb_width', 'thmb_height', 'med_thmb_width', 'med_thmb_height', 'big_thmb_width', 'big_thmb_height', 'pic_max_size', 'pic_max_width', 'pic_max_height', 'watermark_position', 'default_search_view', 'search_type', 'ds_distances_list', 'ds_measuring_unit', 'limit_location_autosuggest', 'gm_api_lang', 'hide_contact_button');
			...
			if($settings['map_type']!='gmaps') {
				$this->tmp["enable_location_autosuggest"]=0;
				$this->tmp["enable_distance_search"]=0;
			}
	...
	function check_form_visibility () {
		...
		if($this->getError()!='')
		{
			...
			// checkbox fields
			$array_checkboxes = array('enable_featured', 'enable_highlited', 'enable_priorities', 'enable_video', 'resize_store_image', 'featured_autoscroll', 'random_priorities', 'prioritize_featured', 'dealer_subdomain', 'enable_urgent', 'enable_url', 'dealers_can_edit_slug');
...
//exit(0);
	...
	function check_form_appearance () {
		...
		if($this->getError()!='')
		{
			...
			$array_fields = array("template", "admin_template", "admin_language", "header_pic", "small_header_pic", "header_pic_link", "footer_pic", "footer_pic_link", "outer_table", "max_cat_per_row", "ads_per_page", "first_page_type", "template_colorscheme", "admin_template_colorscheme");
	...
	function editAppearance() {
		...
		$old_template = $db->fetchAssoc("select template, template_colorscheme, admin_template, admin_template_colorscheme from ".TABLE_APPEARANCE);
		...
		$array_inputs_null = array('template', 'template_colorscheme', 'admin_template', 'admin_template_colorscheme', 'admin_language', 'header_pic_link', 'footer_pic_link', 'outer_table', 'max_cat_per_row', 'ads_per_page', 'first_page_type');
		$i=0;
		foreach ($array_inputs_null as $field) {
		...
		if($old_template['admin_template']!=$this->clean['admin_template'] || (isset($this->clean['admin_template_colorscheme']) && ($old_template['admin_template_colorscheme']!=$this->clean['admin_template_colorscheme'])))
		// empty the cache
	...
	function check_form_seo_settings() {
		...
		$array_sef_links = array("listings", "user_listings", "store", "content", "affiliate", "recent_ads", "featured_ads", "auctions", "login", "register", "logout", "favorites", "contact", "pre_submit", "refine", "contact_details", "new_listing");
	...
	function check_form_settings () {
		...
		if($this->getError()!='')
		{
			...
			// fields with default 0 value
			$array_inputs_zero = array('days_del_expired', 'days_notify', 'delete_login_older_than', 'maps_default_height', 'session_expires', 'accounts_session_timeout');
			...
			// fields with default null value
			$array_inputs_null = array('admin_name', 'admin_username', 'admin_email', 'contact_email', 'site_name', 'maps_default_long', 'maps_default_lat', 'delete_expired', 'subdomain_field', 'recaptcha_public_key', 'recaptcha_private_key', 'google_maps_api_key', 'map_type');
			foreach ($array_inputs_null as $field) {
	...
	function editSettings() {
		...
		// checkbox fields
		$array_checkboxes = array('users_delete_ads', 'users_feature_ads', 'register_captcha', 'login_captcha', 'alerts_captcha', 'contact_captcha', 'cron_simulator', 'send_mail_to_admin_when_pending', 'send_mail_to_admin_when_registeres', 'send_mail_to_admin_when_new_ad', 'send_mail_to_user_when_posting_ads', 'send_mail_to_user_when_expired', 'send_mail_to_user_before_expires', 'nologin_enabled', 'nologin_pending_listing', 'nologin_extra_options', 'nologin_allow_edit', 'nologin_allow_delete', 'nologin_image_verification', 'internal_messaging', 'enable_locations', 'enable_subdomains', 'enable_recaptcha', 'users_can_ask_account_removal', 'contact_messages_pending', 'enable_affiliates', 'nologin_activate_via_email', 'nologin_activate_via_sms');
		...
		// fields with default 0 value
		$array_inputs_zero = array('days_notify', 'delete_login_older_than', 'session_expires', 'accounts_session_timeout');
		...
		if($this->clean['accounts_session_timeout']==0) $this->clean['accounts_session_timeout'] = 24;

		// fields with default null value
		$array_inputs_null = array('admin_username', 'admin_email', 'contact_email', 'subdomain_field', 'recaptcha_public_key', 'recaptcha_private_key', 'google_maps_api_key', 'map_type');
		...
		$array_maps = array('maps_default_long', 'maps_default_lat', 'maps_default_height');
		$no=count($array_maps);
		...
		if(!$this->clean['maps_default_long']) $this->clean['maps_default_long'] = '-83.022206';
		if(!$this->clean['maps_default_lat']) $this->clean['maps_default_lat'] = '39.998264';
		if(!$this->clean['maps_default_height']) $this->clean['maps_default_height'] = '6';
		$i=0;
		foreach ($array_maps as $field) {
			$str_update.=",";
			$str_update.=" `$field` = '".$this->clean[$field]."'";
		...
		if($this->clean['map_type']!='gmaps') {
		
			$db->query("update ".TABLE_ADS_SETTINGS." set `enable_location_autosuggest`=0, `enable_distance_search`=0");
			self::clearSettingsCache();
	...
	function editMobileSettings() {
		...
		// fields with default null value
		$array_inputs_null = array('mobile_template', 'mobile_big_thmb_height', 'mobile_big_thmb_width', 'mobile_thmb_width', 'mobile_thmb_height', 'mobile_header_pic_link', 'mobile_ads_per_page');
		foreach ($array_inputs_null as $field) {
	...
	function editSecuritySettings() {
		...
		// checkbox fields
		$array_checkboxes = array('block_admin_attempts', 'block_user_attempts', 'enable_2fa', '2fa_enable_sms', 'admin_enable_2fa', 'admin_2fa_enable_sms');
		...
		if(isset($_POST["2fa_phone_field"]) && $_POST["2fa_phone_field"]) $this->clean["2fa_phone_field"]=$_POST["2fa_phone_field"]; else $this->clean["2fa_phone_field"]='';
		
		if(isset($_POST["2fa_site_title"]) && $_POST["2fa_site_title"]) $this->clean["2fa_site_title"]=$_POST["2fa_site_title"]; else $this->clean["2fa_site_title"]='';

		if(isset($_POST["admin_phone_number_hidden"]) && $_POST["admin_phone_number_hidden"]) $this->clean["admin_phone_number"]=$_POST["admin_phone_number_hidden"]; else $this->clean["admin_phone_number"]='';

		$res=$db->query('update '.TABLE_SECURITY_SETTINGS.' set `block_admin_attempts` = "'.$this->clean['block_admin_attempts'].'", `allowed_admin_attempts` = "'.$this->clean['allowed_admin_attempts'].'", `block_admin_attempts_for` = "'.$this->clean['block_admin_attempts_for'].'", `block_user_attempts` = "'.$this->clean['block_user_attempts'].'", `allowed_user_attempts` = "'.$this->clean['allowed_user_attempts'].'", `block_user_attempts_for` = "'.$this->clean['block_user_attempts_for'].'", `2fa_site_title` = "'.$this->clean['2fa_site_title'].'", `enable_2fa`= "'.$this->clean['enable_2fa'].'", `2fa_enable_sms`="'.$this->clean['2fa_enable_sms'].'", `admin_enable_2fa`="'.$this->clean['admin_enable_2fa'].'", `admin_2fa_enable_sms`="'.$this->clean['admin_2fa_enable_sms'].'", `2fa_phone_field` = "'.$this->clean['2fa_phone_field'].'", `admin_phone_number` = "'.$this->clean['admin_phone_number'].'";');

		$old_secret = $db->fetchRow("select `admin_2fa_secret` from ".TABLE_SECURITY_SETTINGS);
		$this->clean['admin_2fa_secret'] = $old_secret;
		if($this->clean['admin_enable_2fa'] && !$old_secret) {
			$this->clean['admin_2fa_secret'] = $this->regenerateAdmin2FAKey();
		}
		
		// clear and write cache
		$lc_cache = new cache();
		$lc_cache->writeCache("base_security", $this->clean, '', 1);
	...
	function change_password($remove_2fa = 0) {
		...
		// disable 2FA
		if($remove_2fa)
			$res=$db->query('update '.TABLE_SECURITY_SETTINGS.' set `admin_enable_2fa` = 0;');
	...
	function clearSecuritySettingsCache() {
	
		$lc_cache = new cache();
		$lc_cache->clearCache("base_security");
	...
	public static function emptyCompiled($adm=0) {
		...
		$adm_folder = '';
		if($adm) $adm_folder="/".$config_admin_folder;
...
*/
	...
	function regenerateAdmin2FAKey() {

	
		$ga = new PHPGangsta_GoogleAuthenticator();
		$secret = $ga->createSecret();
		global $db;
		$db->query("update ".TABLE_SECURITY_SETTINGS." set `admin_2fa_secret` = '$secret' ");
