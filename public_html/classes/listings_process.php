...
class listings_process {
	...
	function check_form ($id='') {
		...
		if(!$id) {
 			if(isset($_POST['url']) && $_POST['url']=="on" && !empty($_POST['site_url']) && !validator::valid_url(cleanStr($_POST['site_url']))) $this->addError("site_url", $lng['listings']['error']['invalid_url']); 
		}
		else {
		if(isset($_POST['site_url']) && !empty($_POST['site_url']) && !validator::valid_url(cleanStr($_POST['site_url']))) $this->addError("site_url", $lng['listings']['error']['invalid_url']); 
		}
	...
	function add() {
		...
		if($is_admin || $is_mod) { 
			$u = new users();
			$gr = users::getGroup($clean['user_id']); 
		}
		...
		$clean['date_updated']=$clean['date_added'];

		$insert_array=array("user_id", "package_id", "usr_pkg", "category_id", "currency", "date_added", "date_updated", "active", "pending", "featured", "highlited", "urgent", "priority", "meta_keywords", "meta_description", "language");
		...
		$sql.=$f->add_fields($fieldset, 0, $gr);
		...
		// email_contact
		if($ads_settings['hide_contact_button']==2 && !$is_admin && !$is_mod) {
		
			$email_contact = checkbox_value("email_contact");
			if($email_contact==0) $db->query("update ".TABLE_ADS." set `email_contact` =0 where id='$id'");
	...
	function edit($id) {
		...
		// -------------- PENDING EDITED ---------------
			...
			$f->setEdit($this->edit); // set fields edit mode
			$f->setId($id);
			$field_values=$f->add_fields($fieldset, 1, $gr);
		...
		if($ads_settings['hide_contact_button']==2 && !$is_admin && !$is_mod) {
			...
			$email_contact = checkbox_value("email_contact");
			$db->query("update ".TABLE_ADS." set `email_contact` = $email_contact where id='$id'");
...
// !!!!!! do this when accepting the ad
	...
	function notifyAdminPendingEdited($id) {
		...
		$review_link = $config_live_site.'/'.$config_admin_folder.'/review_pending_edited.php?id='.$id;
