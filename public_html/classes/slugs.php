...
class slugs {
	...
	function replaceSlug($id, $type, $slug) {
...
/*
	...
	function prepareRegenerate($type, $only_missing=0) {

		global $db;
		if(!$only_missing) $db->query("delete from ".TABLE_SLUGS." where `type`='$type'");
	...
	function generateCustomPagesSlugs() {

		global $db;
		$db->query("delete from ".TABLE_SLUGS." where `type` like 'content'");
		...
		$cc = new custom_pages_config();
		$cc->clearNavbarLinksCache();

	}

		function makeMissingSlug($id, $type, $title='') {
		
			if($this->slugForObjectExists($id, $type)) return;
			return $this->makeSlug($id, $type, $title);
