...
class fields {
	...
	function getSearch($type, $set='') {
		...
		if($set)  $where_str .= " and ( `fieldset` REGEXP CONCAT('(^|,)(', REPLACE('$set', ',', '|'), ')(,|$)') or `fieldset` = 0 ) ";
		...
		foreach($array as $row) {
			...
			if($arr[$i]['type']=='depending') {
				...
				$dtable = $dep->getTable($arr[$i]['depending']['table1'], $set);
				foreach($dtable as $key=>$value)
					$dtable[$key]['name_url'] = _urlencode($value['name']);
				$arr[$i]['depending']['elements'] = $dtable;
		...
		foreach($arr as $row) {
			if($arr[$i]['type']=='depending') {
				...
				if(isset($post_arr[$arr[$i]['depending']['caption1']]) && $post_arr[$arr[$i]['depending']['caption1']]) {
					...
					$dep_id1 = $dep->getIdByName($arr[$i]['depending']['table1'], escape($post_arr[$arr[$i]['depending']['caption1']]), " and ( `set_id` REGEXP CONCAT('(^|,)(', REPLACE('$fieldset', ',', '|'), ')(,|$)')  or `set_id` = 0 )");
					$dtable = $dep->getSecondTable($arr[$i]['depending']['table2'], $dep_id1);
					foreach($dtable as $key=>$value)
						$dtable[$key]['name_url'] = _urlencode($value['name']);
					$arr[$i]['depending']['elements2'] = $dtable;
				...
				if($arr[$i]['depending']['no']>2 && isset($post_arr[$arr[$i]['depending']['caption2']]) && $post_arr[$arr[$i]['depending']['caption2']]) {
					...
					$dtable = $dep->getSecondTable($arr[$i]['depending']['table3'], $dep_id2);
					foreach($dtable as $key=>$value)
						$dtable[$key]['name_url'] = _urlencode($value['name']);
					$arr[$i]['depending']['elements3'] = $dtable;
				...
				if($arr[$i]['depending']['no']>3 && isset($post_arr[$arr[$i]['depending']['caption3']]) && $post_arr[$arr[$i]['depending']['caption3']]) {
					...
					$dtable = $dep->getSecondTable($arr[$i]['depending']['table4'], $dep_id3);
					foreach($dtable as $key=>$value)
						$dtable[$key]['name_url'] = _urlencode($value['name']);
					$arr[$i]['depending']['elements4'] = $dtable;
				...
				if($arr[$i]['depending']['no']>4 && isset($post_arr[$arr[$i]['depending']['caption4']]) && $post_arr[$arr[$i]['depending']['caption4']]) {
					...
					$dtable = $dep->getSecondTable($arr[$i]['depending']['table5'], $dep_id4);
					foreach($dtable as $key=>$value)
						$dtable[$key]['name_url'] = _urlencode($value['name']);
					$arr[$i]['depending']['elements5'] = $dtable;
	...
	function getAll($set='', $set2='') {

		global $db;
		global $crt_lang, $is_admin;

		$where_str="";
		if($set && $this->type=="cf") $where_str .= " and ( fieldset REGEXP CONCAT('(^|,)(', REPLACE('$set', ',', '|'), ')(,|$)') or fieldset = 0 ) ";
		if($set==-1&& $this->type=="uf") $where_str .= " and `groups` = $set";
		if($set && $set!=-1 && $this->type=="uf") $where_str .= " and ( (`groups` REGEXP CONCAT('(^|,)(', REPLACE('$set', ',', '|'), ')(,|$)') and `groups`!=-1)  or `groups` = 0 )";
		if($set2 && $this->type=="cf") $where_str .= " and ( (`groups` REGEXP CONCAT('(^|,)(', REPLACE('$set2', ',', '|'), ')(,|$)'))  or `groups` = 0 )";
		...
		foreach($array as $row) {
			...
			if($this->type=='cf' || ($this->type=='uf' && (!$set || $row['groups']==0 || in_array($set, $arr_gr)))) {
				...
				// numeric format
				$arr[$i]['format_decimals'] = $arr[$i]['format_point'] = $arr[$i]['format_separator'] = '';
	...
	function getFieldsArray($set, $public=1) {
		...
		$extra="";
		$where_str='';
		if($this->type=='cf') { $where_str=" and (fieldset REGEXP CONCAT('(^|,)(', REPLACE('$set', ',', '|'), ')(,|$)') or `fieldset` = 0)"; $extra = ", `dep_id`"; }

		else {
			if($public) $where_str=" and `public`>=1"; else $where_str="";
			if($set!=-1)
				$where_str.=" and ( (`groups` REGEXP CONCAT('(^|,)(', REPLACE('$set', ',', '|'), ')(,|$)') and `groups` != -1 ) or `groups` = '0')";
	...
	function gMapsFields($set) {
		...
		global $db;
		if($this->type=="cf") {
			$sql = "select caption, extensions from `".$this->table."` where (fieldset REGEXP CONCAT('(^|,)(', REPLACE('$set', ',', '|'), ')(,|$)') or `fieldset`= 0) and active=1 and `type` like 'google_maps'";
	...
	function getGmapsField($set) {
		...
		global $db;
		$sql = "select caption from `".$this->table."` where (fieldset REGEXP CONCAT('(^|,)(', REPLACE('$set', ',', '|'), ')(,|$)') or `fieldset`= 0) and active=1 and `type` like 'google_maps'";
		$q = $db->query($sql);
		$exists = $db->numRows($q);
		if(!$exists) return 0;
	...
	function HTMLAreaFieldExists($set) {
		...
		global $db;
		if($this->type=="cf") {
			$sql = "select id from `".$this->table."` where (fieldset REGEXP CONCAT('(^|,)(', REPLACE('$set', ',', '|'), ')(,|$)') or `fieldset`= 0) and active=1 and `type` like 'htmlarea'";
			$q = $db->query($sql);
			$exists = $db->numRows($q);
			return $exists;
		}
	...
	function getListingSpecialCF($fset) {
		...
		global $settings;
		$special_cf = array("phone" =>array(), "whatsapp" => array(), "twitter" => array(), "logo" => array(), "button" => array());
		$phone_idx = 0; $whatsapp_idx=0; $twitter_idx=0; $logo_idx=0; $button_idx=0;
		if(!$fset) $fset=0;
		...
		foreach($array_fields as $field) {
			...
			if($field['type']=="button")
				$special_cf['button'][$button_idx++] = array ("caption" => $field['caption']);
	...
	function getAdminVerificationExists() {

			global $db;
			$exists = $db->fetchRow("select count(*) from ".$this->table." where `admin_verification`=1 and active=1");
			return $exists;
	}
