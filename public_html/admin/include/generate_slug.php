...
if(isset($_GET['only_missing'])) $only_missing=$_GET['only_missing']; else $only_missing=0;

if(isset($_GET['crt']) && is_numeric($_GET['crt'])) 
	$crt = escape($_GET['crt']);
else { // first id
	$crt = $s->prepareRegenerate($type, $only_missing);
	if($only_missing) $s->makeMissingSlug($crt, $type);
	else $s->makeSlug($crt, $type);
	//echo $crt;
}
...
foreach($array_id as $id) {
	...
	if($only_missing) $s->makeMissingSlug($id, $type);
	else $s->makeSlug($id, $type);
