...
function common($smarty, $page="") {
	...
	global $config_live_site, $config_abs_path, $config_demo, $config_data_set, $config_admin_folder;
	$smarty->compile_dir=$config_abs_path.'/'.$config_admin_folder.'/temp/';
	$smarty->assign("live_site",$config_live_site);
	$smarty->assign("abs_path",$config_abs_path);
	$smarty->assign("demo", $config_demo);
	$smarty->assign("admin_folder", $config_admin_folder);
...
//$is_admin=1;
	...
	if(!$is_admin && !$is_mod) { header("Location: ".$config_live_site."/login.php?loc=".$config_admin_folder."/".$self); exit(0); }
	...
	if($is_mod && $self_noext!="not_authorized") {
		...
		if($self_noext!="index" && $self_noext!="get_info" && $self_noext!="actions") {
			...
			if(!$page_section) { header("Location: ".$config_live_site."/".$config_admin_folder."/not_authorized.php"); exit(0); }
			if( $page_section!= "any" && $mod_sections[$page_section][$page_action]==0) {
				...
				foreach($fwd_to as $key => $value) {
					...
					if($self_noext == $key) { header("Location: ".$config_live_site."/".$config_admin_folder."/".$value.".php"); exit(0);  }

				}
				header("Location: ".$config_live_site."/".$config_admin_folder."/not_authorized.php"); exit(0);
			}
		}
	}
...
//_print_r($ads_settings);
	...
	$smarty->template_dir = $config_abs_path."/".$config_admin_folder."/templates/".$appearance_settings['admin_template'];
	$smarty->assign("template_path",$config_live_site."/".$config_admin_folder."/templates/".$appearance_settings['admin_template']."/");
