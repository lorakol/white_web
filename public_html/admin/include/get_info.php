...
$arr_action= array("package", "categ", "banners", "listing", "pkg", "cf", "uf", "dep", "group", "depending", "positions", "fieldset", "rule", "user_ac", "tpl", "admin_tpl");
...
//***
	...
	case 'admin_tpl':
		...
		$tpl = escape($_GET['tpl']);
		global $db, $config_table_prefix;
		$arr = $db->fetchRowList("select `colorscheme` from ".$config_table_prefix."admin_tpl_colorschemes where `tpl`='$tpl'");
		if(!$arr) echo "0"; 
		else echo implode("|",$arr);
