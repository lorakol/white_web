...
$only_missing=0;
if(isset($_GET['only_missing']) && $_GET['only_missing']==1) $only_missing=1;

if(isset($_GET['image']) && is_numeric($_GET['image'])) 
	$image = escape($_GET['image']); 
else { // first image
	$image = $pic->prepareRegenerate($only_missing);
}
$watermark = 0;
if($_GET['watermark']==0 || $_GET['watermark']==1) $watermark = $_GET['watermark'];

$pic->regeneratePicture($image, $watermark, $only_missing);
