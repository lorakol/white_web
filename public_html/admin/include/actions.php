...
$arr_action= array("enable", "disable", "block", "unblock", "flag", "unflag", "delete", "move_up", "move_down", "clear_hits", "sold", "rented", "unsold", "unrented", "feature", "unfeature", "renew", "accept", "deny", "accept_featured", "install", "uninstall", "default", "pending", "not_pending", "edit", "save", "move", "delete_file", "send", "mark_paid", "whitelist", "resend", "make", "exists");
$arr_object= array("user", "group", "navbar", "cf", "categ", "banner_categ", "banners", "listing", "picture", "order", "custom_page", "pkg", "uf", "currency", "pri", "usr_pkg", "module", "rss", "language", "payment", "ie_template", "db", "store", "bulk_uploads", "info", "mail", "processor_title", "processor_recurring", "coupon", "fieldset", "search", "alert", "msg", "rule", "fortumo", "credits", "sch", "rev", "aff_payment", "pe", "blockedip", "flaggedip", "whitelistedip", "blockedemail", "whitelistedemail", "smsg", "user_sms", "listing_sms", "invoice", "blockedphone", "whitelistedphone", "tpay", "mobilpay", "slug", "fp", "aver");
...
switch ($object) {
	...
	case 'user':
		...
		if($is_admin || $mod_sections['users']['edit']) {
			...
			else if($action == "unflag") $usr->Unflag($id);
		}
...
*/
	...
	case 'msg':
		...
		else if($is_admin || $mod_sections['messages']['edit']) {

			if($action=="block") $msg->Block($id);
			else if($action=="unblock") $msg->Unblock($id);
			if($action=="flag") $msg->Flag($id);
			else if($action=="unflag") $msg->Unflag($id);
			else if($action=="send") {
	...
	case 'flaggedip':
		...
		if(!strcmp($action,"whitelist")  && ($is_admin || $mod_sections['security']['edit']) ) {
			if(isset($_GET['ip'])) $ip=escape($_GET['ip']); else exit(0);
			$fi->addToWhitelist($ip);
		}
		if(!strcmp($action,"block")  && ($is_admin || $mod_sections['security']['edit']) ) {
			if(isset($_GET['ip'])) $ip=escape($_GET['ip']); else exit(0);
			$fi->Block($ip);
		}
	...
	case 'aver':
		...
		$id = get_numeric_only("id");
		if(isset($_GET['caption']) && $_GET['caption']) $caption = escape($_GET['caption']); else exit(0);
		if(isset($_GET['ftype']) && $_GET['ftype']) $ftype = escape($_GET['ftype']); else $ftype="uf";

		if($is_admin || $mod_sections['users']['edit']) {
			$fp = new fields_process($ftype);
			if($action == "accept") $fp->acceptAdminVerification($id, $caption);
			else if($action == "deny") $fp->denyAdminVerification($id, $caption);
		}
