...
function mapStatus(display, hide) {
	...
	var disp = document.getElementById("maps_location").style.display;
	if(disp=="none") { 
		document.getElementById("maps_location").style.display="block";
		document.getElementById("display_map").innerHTML = '<a href="javascript:;" onClick="mapStatus(\''+display+'\', \''+hide+'\')">'+hide+'</a><br><br>';
		if($('input[name=map_type]:checked', '#settings').val()=='osm')
			init_osm();
		else
			load_gmaps();
	}
	else { 
		document.getElementById("maps_location").style.display = "none";
...
function onBlock(ip, confirm_str) {

	if(confirm_str != undefined && confirm_str != '' && myConfirm(confirm_str)==false) return false;

	var url_str="include/actions.php?object=flaggedip&action=block&ip="+ip;
